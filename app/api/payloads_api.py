...
from app.database_models.model import Operator, Payload, Callback, C2Profile, PayloadType, Operation
...
async def register_payload_func(data):
    ...
    try:
        ...
        return {**status, **pload_json}
    except Exception as e:
        if "unique constraint \"payload_uuid\"" not in str(e):
            # just tried to create a duplicate payload, just return the current one
            print(e)
