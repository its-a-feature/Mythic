...
async def perform_load_transforms(data, cb, operation, op):
    try:
        ...
        if load_transforms['status'] == "success":
            ...
            for file in glob.glob(r'./app/c2_profiles/{}/{}/{}/*'.format(operation.name,
            ...
            # make sure we have the latest pulled info for the transform functions
            query = await db_model.payload_query()
            registered_payload = await db_objects.get(query, id=cb.registered_payload)
            for t in load_transforms['transforms']:
                try:
                    transform_output = await getattr(transforms, t['name'])(registered_payload,
