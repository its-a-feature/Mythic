...
# -------  API Tokens FUNCTION -----------------
@apfell.route(apfell.config['API_BASE'] + "/apitokens", methods=['GET'])
@inject_user()
...
@apfell.route(apfell.config['API_BASE'] + "/apitokens", methods=['POST'])
@inject_user()
...
async def create_apitokens(request, user):
    ...
    if 'token_type' not in data or data['token_type'] not in ['C2', 'User']:
        return json({'status': 'error', 'error': 'token_type must be specified and be "C2" or "User"'})
    try:
        query = await db_model.operator_query()
        operator = await db_objects.get(query, username=user['username'])
        token = await request.app.auth.generate_access_token({'user_id': user['user_id'], 'auth': "apitoken", 'token_type': data['token_type']})
        try:
            apitoken = await db_objects.create(db_model.APITokens, token_type=data['token_type'],
                                               token_value=token, operator=operator)
...
@apfell.route(apfell.config['API_BASE'] + "/apitokens/<tid:int>", methods=['PUT'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@apfell.route(apfell.config['API_BASE'] + "/apitokens/<tid:int>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
