exports.test_password = function(task, command, params){
    ...
    if(user.js != undefined){
        if(user.authenticateWithPassword($(password))){
            return JSON.stringify({"user_output":"Successful authentication", "completed": true});
        }
        else{
            return JSON.stringify({"user_output":"Failed authentication", "completed": true});
        }
    }
    else{
        return JSON.stringify({"user_output":"User does not exist", "completed": true, "status": "error"});
    }
};
COMMAND_ENDS_HERE