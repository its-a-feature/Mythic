exports.persist_emond = function(task, command, params){
    try{
        ...
        if(payload_type === "oneliner-jxa"){
            ...
            return JSON.stringify({"user_output":"command is a required field for the custom_bash-c payload_type", "completed": true, "status": "error"});
            }
        }
        // get our new plist file_name
        if(config.hasOwnProperty('file_name') && config['file_name'] != ""){ var file_name = config['file_name'];}
        else{ return JSON.stringify({"user_output":"file name is required", "completed": true, "status": "error"}); }
    ...
    "</plist>";
        ...
        if(queueDirectories != undefined && queueDirectories.length > 0){
            ...
            let artifacts = JSON.stringify({'user_output': user_output, 'artifacts': [{'base_artifact': 'File Create', 'artifact': queueDirectoryPath + "/.DS_Store"}, {'base_artifact': 'File Create', 'artifact': '/etc/emond.d/rules/' + file_name}], "completed": true});
            return artifacts
        }
        else{
            return JSON.stringify({"user_output":"QueueDirectories array is either not there or 0 in length", "completed": true, "status": "error"});
        }
    }catch(error){
