...
from app.database_models.model import operator_query, operation_query, operatoroperation_query, apitokens_query
import datetime
import json
...
async def add_scopes_to_payload(user, *args, **kwargs):
    ...
    try:
        query = await operatoroperation_query()
        operationsmap = await db_objects.execute(query.where(OperatorOperation.operator == dbuser))
        if dbuser.admin:
            scopes.append('admin')
