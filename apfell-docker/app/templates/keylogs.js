            ...
            for(let i in this.keylogs){ // i is group
                for(let j in this.keylogs[i]){ // j is subgroup
                    for(let k in this.keylogs[i][j]){ // k is window titles
                        let combo = Object.keys(this.keylogs[i][j][k]['keylogs']).map(function(x){return [keylog.keylogs[i][j][k]['keylogs'][x]['keystrokes']];}).join('');
...
let ws = new WebSocket('{{ws}}://{{links.server_ip}}:{{links.server_port}}/ws/keylogs/current_operation');
    ws.onmessage = function(event){
        if (event.data !== ""){
            ...
            alertTop("success", "<b>New Keylog from " + k['task'] + ": </b><pre>" + key_stroke_alert + "</pre>", 2);
            //console.log(rsp);
            if(keylog.keylogs.hasOwnProperty(group)){
                if(keylog.keylogs[group].hasOwnProperty(group2)){
                    if(keylog.keylogs[group][group2].hasOwnProperty(window)){
                        keylog.keylogs[group][group2][window]['keylogs'].push(k);
                        if(!keylog.keylogs[group][group2][window].hasOwnProperty('apfell_badges')){
