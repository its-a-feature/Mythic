        ...
        try:
            ...
            if command == "start":
                if not running:
                    ...
                    output = ""
                    process = subprocess.Popen(str(server_path), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=str(container_files_path))
                ...
                if running:
                    ...
                    try:
                        ...
                        pass
                    running = False
                    await send_status(message="Process killed...\nOld Output: {}".format(output), routing_key="c2.status.{}.stopped.stop.{}".format(hostname,username))
                    output = ""
                else:
                    await send_status(message="Process not running...\nOld Output: {}".format(output), routing_key="c2.status.{}.stopped.stop.{}".format(hostname,username))
                    output = ""
                # make sure to stop the /Apfell/server in the background
            elif command == "status":
                if running:
                    await send_status(message="Output: {}".format(output), routing_key="c2.status.{}.running.status.{}".format(hostname,username))
                    output = ""
                else:
                    await send_status(message="C2 is not running", routing_key="c2.status.{}.stopped.status.{}".format(hostname,username))
                ...
                encoded_data = json.dumps(
                    {"filename": "config.json", "data": base64.b64encode(file_data).decode('utf-8')})
                await send_status(message=encoded_data, routing_key="c2.status.{}.{}.get_config.{}".format(hostname, "running" if running else "stopped", username))
                ...
                try:
                    ...
                    if container_files_path not in file_path.parents:
                        response = {'status': 'error', 'error': 'trying to break out of path'}
                    else:
                ...
                await send_status(message=json.dumps(response),
                ...
                try:
                    ...
                    for cls in C2Profile.__subclasses__():
                        ...
                        break
                    await send_status(message=json.dumps(c2profile),
                                      routing_key="c2.status.{}.{}.sync_classes.{}".format(hostname, "running" if running else "stopped", username))
                except Exception as e:
                    await send_status(message='{"message": "Error while syncing info: {}"}'.format(str(traceback.format_exc())),
                                      routing_key="c2.status.{}.{}.sync_classes.{}".format(hostname, "running" if running else "stopped", username))
            else:
...
async def sync_classes():
    try:
        ...
        for cls in C2Profile.__subclasses__():
            ...
            break
        await send_status(message=json.dumps(c2profile),
                          routing_key="c2.status.{}.{}.sync_classes.{}".format(hostname, "stopped", ""))
    except Exception as e:
        await send_status(message='{"message": "Error while syncing info: {}"}'.format(str(traceback.format_exc())),
                          routing_key="c2.status.{}.{}.sync_classes.{}".format(hostname, "stopped", ""))
...
async def rabbit_c2_rpc_callback(exchange: aio_pika.Exchange, message: aio_pika.IncomingMessage):
    with message.process():
        ...
        try:
            ...
            print("Exception trying to send message back to container for rpc! " + str(e))
            sys.stdout.flush()
...
async def mythic_service():
    ...
    container_files_path = pathlib.Path(os.path.abspath(main_config['container_files_path']))
    ...
    try:
        ...
        await queue.bind(exchange='mythic_traffic', routing_key="c2.modify.{}.#".format(hostname))
