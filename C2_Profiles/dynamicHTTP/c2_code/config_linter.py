...
def check_server_layout(server_config):
    if 'instances' not in server_config:
        print(f"{bcolors.FAIL}[-]{bcolors.ENDC} config.json must start with \"instances\"")
    ...
    for inst in server_config['instances']:
        ...
        for method in ['GET', 'POST']:
            if method not in inst:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"{method}\" element in instance")
                sys.exit(1)
            if 'ServerBody' not in inst[method]:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"ServerBody\" array in {method}")
                sys.exit(1)
            for f in inst[method]['ServerBody']:
                if 'function' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"function\" name in {method} ServerBody")
                    sys.exit(1)
                if 'parameters' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"parameters\" array in {method} in ServerBody (can be an empty array indicated by []")
                    sys.exit(1)
            ...
            if len(inst[method]['AgentMessage']) == 0:
                print(f"[*] \"AgentMessage\" array is empty, so you won't be able to do {method} messages")
            for m in inst[method]['AgentMessage']:
                if 'urls' not in m:
                    print("[-] Missing \"urls\" array indicating urls where the agent will reach out to")
                    sys.exit(1)
                if 'uri' not in m:
                    print("[-] Missing \"uri\" indicator of what the URI will be. If not in use, set to empty string")
                    sys.exit(1)
                if 'urlFunctions' not in m:
                    print("[-] Missing \"urlFunctions\" array, if you don't intent to do any manipulations here, set to empty array []")
                    sys.exit(1)
                for f in m['urlFunctions']:
                    if 'name' not in f:
                        ...
                        sys.exit(1)
                    if 'value' not in f:
                        print("[-] Missing \"value\" parameter in urlFunction. This is the starting value before transforms are applied")
                        sys.exit(1)
                    if 'transforms' not in f:
                        print("[-] Missing \"transforms\" array. If no transforms needed, set to empty array []")
                        sys.exit(1)
                    for t in f['transforms']:
                        if 'function' not in t:
                            print("[-] Missing \"function\" name in transforms in urlFunctions")
                            sys.exit(1)
                        if 'parameters' not in t:
                            print("[-] Missing \"parameters\" array in transforms in urlFunctions (can be an empty array indicated by []")
                            sys.exit(1)
                if 'AgentHeaders' not in m:
                    print("[-] Missing \"AgentHeaders\" dictionary, this can be blank if the agent won't set any headers (i.e. {}")
                    sys.exit(1)
                if 'QueryParameters' not in m:
                    print("[-] Missing \"QueryParameters\" array in GET. If no query parameters will be set, leave as empty array []")
                    sys.exit(1)
                for f in m['QueryParameters']:
                    if 'name' not in f:
                        ...
                        sys.exit(1)
                    if 'value' not in f:
                        print("[-] Missing \"value\" parameter in QueryParameters. This is the starting value before transforms are applied")
                        sys.exit(1)
                    if 'transforms' not in f:
                        print("[-] Missing \"transforms\" array. If no transforms needed, set to empty array []")
                        sys.exit(1)
                    for t in f['transforms']:
                        if 'function' not in t:
                            print("[-] Missing \"function\" name in transforms in QueryParameters")
                            sys.exit(1)
                        if 'parameters' not in t:
                            print("[-] Missing \"parameters\" array in transforms in QueryParameters (can be an empty array indicated by []")
                            sys.exit(1)
                if 'Cookies' not in m:
                    print("[-] Missing \"Cookies\" array in GET. If none will be set, leave as empty array []")
                    sys.exit(1)
                for f in m['Cookies']:
                    if 'name' not in f:
                        ...
                        sys.exit(1)
                    if 'value' not in f:
                        print("[-] Missing \"value\" parameter in Cookies. This is the starting value before transforms are applied")
                        sys.exit(1)
                    if 'transforms' not in f:
                        print("[-] Missing \"transforms\" array. If no transforms needed, set to empty array []")
                        sys.exit(1)
                    for t in f['transforms']:
                        if 'function' not in t:
                            print("[-] Missing \"function\" name in transforms in Cookies")
                            sys.exit(1)
                        if 'parameters' not in t:
                            print("[-] Missing \"parameters\" array in transforms in Cookies (can be an empty array indicated by []")
                            sys.exit(1)
                if 'Body' not in m:
                    print("[-] Missing \"Body\" array in GET message. If none will be supplied, set as empty array []")
                    sys.exit(1)
        ...
        if 'action' not in inst['no_match']:
            ...
            sys.exit(1)
        if inst['no_match']['action'] not in ["redirect", "proxy_get", "proxy_post", "return_file"]:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} no_match action isn't in the approved list")
            sys.exit(1)
        if 'redirect' not in inst['no_match']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"redirect\" option in no_match")
            sys.exit(1)
        if 'proxy_get' not in inst['no_match']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"proxy_get\" option in no_match")
            sys.exit(1)
        if 'url' not in inst['no_match']['proxy_get']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"url\" in no_match's proxy_get dictionary")
            sys.exit(1)
        if 'status' not in inst['no_match']['proxy_get']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"status\" code for no_match's proxy_get dictionary")
            sys.exit(1)
        if 'proxy_post' not in inst['no_match']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"proxy_post\" option in no_match")
            sys.exit(1)
        if 'url' not in inst['no_match']['proxy_post']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"url\" in no_match's proxy_post dictionary")
            sys.exit(1)
        if 'status' not in inst['no_match']['proxy_post']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"status\" code in no_match's proxy_post dictionary")
            sys.exit(1)
        ...
        if "name" not in inst['no_match']['return_file']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"name\" for the file to be returned in no_match case")
            sys.exit(1)
        if not os.path.exists(inst['no_match']['return_file']['name']):
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} File specified in \"no_match\" case for \"return_file\" can't be found")
            sys.exit(1)
        if 'status' not in inst['no_match']['return_file']:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Misisng \"status\" return code for no_match's return_file")
            sys.exit(1)
        if 'port' not in inst:
        ...
        if 'debug' not in inst:
            print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"debug\" boolean in instance")
            sys.exit(1)
...
def check_agent_config_layout(inst):
    for method in ['GET', 'POST']:
        ...
        for m in inst[method]['AgentMessage']:
            if 'urls' not in m:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"urls\" array indicating urls where the agent will reach out to")
                sys.exit(1)
            if 'uri' not in m:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"uri\" indicator of what the URI will be. If not in use, set to empty string")
                sys.exit(1)
            if 'urlFunctions' not in m:
                print(
            ...
            for f in m['urlFunctions']:
                if 'name' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"name\" parameter in urlFunction")
                    sys.exit(1)
                if 'value' not in f:
                    print(
                    ...
                    sys.exit(1)
                if 'transforms' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"transforms\" array. If no transforms needed, set to empty array []")
                    sys.exit(1)
                for t in f['transforms']:
                    if 'function' not in t:
                        print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"function\" name in transforms in urlFunctions")
                        sys.exit(1)
                    if 'parameters' not in t:
                        print(
            ...
            for f in m['QueryParameters']:
                if 'name' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"name\" parameter in QueryParameters")
                    sys.exit(1)
                if 'value' not in f:
                    print(
                ...
                for t in f['transforms']:
                    if 'function' not in t:
                        print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"function\" name in transforms in QueryParameters")
                        sys.exit(1)
                    if 'parameters' not in t:
                        print(
                        ...
                        sys.exit(1)
            if 'Cookies' not in m:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"Cookies\" array in GET. If none will be set, leave as empty array []")
                sys.exit(1)
            for f in m['Cookies']:
                if 'name' not in f:
                    print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"name\" parameter in Cookies")
                    sys.exit(1)
                if 'value' not in f:
                    print(
                ...
                for t in f['transforms']:
                    if 'function' not in t:
                        print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"function\" name in transforms in Cookies")
                        sys.exit(1)
                    if 'parameters' not in t:
                        print(
                        ...
                        sys.exit(1)
            if 'Body' not in m:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Missing \"Body\" array in GET message. If none will be supplied, set as empty array []")
                sys.exit(1)
...
def check_config(server_config, agent_config, method):
    ...
    for g in agent_config[method]['AgentMessage']:
        ...
        print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Current URLs: {g['urls']}\n\tCurrent URI: {g['uri']}")
        for p in g['QueryParameters']:
            if p['value'] == "message":
                print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Found 'message' keyword in QueryParameter {p['name']}")
                agent_message['location'] = "QueryParameters"
                agent_message['value'] = p
        for p in g['Cookies']:
            if p['value'] == 'message':
                print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Found 'message' keyword in Cookies {p['name']}")
                agent_message['location'] = "Cookies"
                agent_message['value'] = p
        for p in g['urlFunctions']:
            if p['name'] == '<message:string>':
                print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Found '<message:string>' keyword in urlFunctions")
                agent_message['location'] = "URI"
                agent_message['value'] = p
        if agent_message['location'] == "":
            # if we haven't set it yet, data must be in the body
            print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Didn't find message keyword anywhere, assuming it to be the Body of the message")
            agent_message['location'] = "Body"
            agent_message['value'] = g['Body']
        print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Now checking server config for matching section")
    ...
    for inst in server_config['instances']:
        ...
        for g in inst[agent_message['method']]['AgentMessage']:
            ...
            if not urls_match(agent_message['urls'], g['urls']):
                ...
                print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Found matching URLs and URI, checking rest of AgentMessage")
            if agent_message['location'] == "Body":
                print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Checking for matching Body messages")
                match = body_match(g['Body'], agent_message['value'])
            else:
                match = contains_element(agent_message['value'], g[agent_message['location']])
            if match:
                print(f"{bcolors.OKGREEN}[+]{bcolors.ENDC} FOUND MATCH")
                return True
            else:
                print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Matched URLs/URI failed to match AgentMessages")
                return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(f"{bcolors.FAIL}[-]{bcolors.ENDC} Please specify an agent config file on the command line")
        sys.exit(1)
    ...
    print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Checking server config for layout structure")
    check_server_layout(server_config)
    print(f"{bcolors.OKGREEN}[+]{bcolors.ENDC} Server config layout structure is good")
    print(f"{bcolors.OKBLUE}[*]{bcolors.ENDC} Checking agent config for layout structure")
