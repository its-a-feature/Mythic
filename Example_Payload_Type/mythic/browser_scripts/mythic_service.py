...
from importlib import util, reload, invalidate_caches, import_module
...
def import_all_agent_functions():
    ...
    for x in modules:
        if not x.endswith("__init__.py") and x[-3:] == ".py":
            ...
            reload(module)
            #mod_name = "agent_functions." + Path(x).stem
            #if mod_name in globals():
            #    print("trying to reload {}".format(str(x)))
            #    sys.stdout.flush()

            #else:
            #    print("trying to load {}".format(str(x)))
            #    sys.stdout.flush()
            #    spec = util.spec_from_file_location(mod_name, x)
            ...
            #    print(obj)
            #    if obj in globals():
            #        print("above obj already exists in globals")
            #        sys.stdout.flush()
    ...
    with message.process():
        ...
        if command == "create_payload_with_code":
            try:
                ...
                try:
                    ...
                    resp_message = {"status": "error",
                                    "message": "Error in agent creation: " + str(traceback.format_exc()),
                                    "payload": ""}
                    await send_status(json.dumps(resp_message),
                                      "create_payload_with_code", "{}".format(username))
                    return
                ...
                resp_message = {"status": build_resp.get_status().value,
                                "message": build_resp.get_message(),
                                "build_parameter_instances": build_instances,
                                "payload": base64.b64encode(build_resp.get_payload()).decode('utf-8')}
                await send_status(json.dumps(resp_message),
                                      "create_payload_with_code", "{}".format(username))
            ...
            try:
                ...
                for cls in CommandBase.__subclasses__():
                    if getattr(cls, 'cmd') == message_json['command']:
                        ...
                        task = MythicTask(message_json['task'], args=Command.argument_class(message_json['params']))
                        ...
                        await send_status(str(final_task),
                            "command_transform", "{}.{}".format(final_task.status.value, pieces[4]), username)
                        break
                if final_task is None:
                    await send_status("Failed to find class where command_name = " + message_json['command'],
                                      "command_transform", "error.{}".format(pieces[4]), username)
            except Exception as e:
                await send_status("Error while creating/running create_tasking: " + str(traceback.format_exc()),
                                  "command_transform", "error.{}".format(pieces[4]), username)
                return
            ...
            try:
                ...
                await send_status(json.dumps(payload_type),
                                  "sync_classes", "success", username)
            except Exception as e:
                await send_status("Error while syncing info: " + str(traceback.format_exc()),
                                  "sync_classes", "error.{}".format(pieces[4]), username)
        else:
...
async def mythic_service():
    ...
    try:
        ...
        await queue.bind(exchange='mythic_traffic', routing_key="pt.task.{}.#".format(hostname))
