...
func setMythicConfigDefaultValues() {
	...
	mythicEnv.SetDefault("mythic_docker_image_mirror", "")
	mythicEnvInfo["mythic_docker_image_mirror"] = `This sets an internal Docker registry to pull images from instead of their original upstream registries. When empty (the default), every image is pulled from its original location (for example, "ghcr.io/its-a-feature/mythic_server:<global_docker_latest>" for core services, or whatever URL is declared in each installed agent's/C2 profile's config.json "remote_images" entry). When set to a non-empty value, the leading registry host of every image is replaced with this value while the rest of the path is preserved. For example, with mythic_docker_image_mirror="myregistry.local/mirror", the core image "ghcr.io/its-a-feature/mythic_server:0.0.5" becomes "myregistry.local/mirror/its-a-feature/mythic_server:0.0.5" and an agent image "ghcr.io/mythicagents/apollo:latest" becomes "myregistry.local/mirror/mythicagents/apollo:latest". The same rewrite is applied every time the docker-compose file is generated, so this can be changed and applied with start/build without re-installing services. This setting has no effect on services where *_use_build_context is true. Images without a registry host (for example, "nginx:latest") are treated as having no host and the full path is preserved under the mirror.`
...
It's unlikely this will ever need to change since you should be connecting through the nginx_proxy, but would be necessary to change if the React UI were hosted on a different server.`
	...
	mythicEnvInfo["mythic_react_use_volume"] = `This specifies whether the mythic_react container uses named Docker volumes for its Nginx configuration and compiled UI.
When false, the local Nginx configuration is mounted into the container. The local compiled UI is mounted only when mythic_react_use_build_context is true; otherwise, the UI baked into the published image is used.`

	mythicEnv.SetDefault("mythic_react_use_build_context", false)
	mythicEnvInfo["mythic_react_use_build_context"] = `This specifies whether the mythic_react container uses the pre-built image hosted on GitHub's container registry (ghcr.io) or builds the local mythic-react-docker/Dockerfile.
When true and mythic_react_use_volume is false, the ignored output generated by mythic-cli build_ui is mounted into the local container.`
...
After making changes to the UI, run 'sudo ./mythic-cli build_ui' to compile them into the ignored mythic-react-docker/mythic/public folder.
With mythic_react_use_build_context set to true and mythic_react_use_volume set to false, disabling debug mode serves that locally compiled UI.`
