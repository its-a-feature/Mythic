...
func GitClone(url string, branch string) (string, error) {
	...
	err = runGitClone([]string{"clone", "--depth", "1", "--recurse-submodules", "--single-branch", "--branch", branch, url, filepath.Join(workingPath, "tmp")}, false)
	if err != nil {
		return "", err
	}
...
func InstallService(url string, branch string, overWrite bool, keepVolume bool) (error, map[string]string) {
	...
	if branch == "" {
		...
		err = runGitClone([]string{"clone", "--depth", "1", "--recurse-submodules", "--single-branch", url, filepath.Join(workingPath, "tmp")}, true)
	} else {
		log.Printf("[*] Cloning branch \"%s\" from %s\n", branch, url)
		err = runGitClone([]string{"clone", "--depth", "1", "--recurse-submodules", "--single-branch", "--branch", branch, url, filepath.Join(workingPath, "tmp")}, true)
	}
	if err != nil {
