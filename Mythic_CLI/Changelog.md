...
- Added `jupyter_include` as an option to _not_ include the jupyter notebook docker image in the docker-compose.yml file
- Made the `docker-compose.yml` write atomic friendly with a temp write + replace

## [0.3.26] - 2026-03-16

### Changed

- Updated the docker SDK version
...
- Updated to properly remove all unused images on the system when starting a container
- Added new `./mythic-cli remove_images` command to explicitly remove unused images
- Added `depends_on` clauses to services
...
- Merged PR from Norton1999 allowing git@ prefix in addition to http for remote git tracking
