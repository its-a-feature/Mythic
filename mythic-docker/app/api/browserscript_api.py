...
async def create_browserscript(request, user):
    ...
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot create browserscripts'})
    ...
    if 'command' in data:
        ...
        try:
            ...
            if 'name' in data:
                ...
                return json({"status": 'error', 'error': 'Missing name for support script'})
        except Exception as e:
...
async def modify_browserscript(request, user, bid):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot modify browser scripts'})
        data = request.json
    except Exception as e:
        return json({'status': 'error', 'error': 'failed to parse json'})
    try:
        ...
        if 'script' in data:
            if browserscript.container_version != data['script'] and browserscript.container_version != "":
...
@mythic.route(mythic.config['API_BASE'] + "/browser_scripts/<bid:int>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_browserscript(request, user, bid):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot remove browser scripts'})
        query = await db_model.operator_query()
        operator = await db_objects.get(query, username=user['username'])
        query = await db_model.browserscript_query()
...
async def set_default_scripts(new_user):
    try:
        ...
        scripts = await db_objects.execute(script_query.where(db_model.BrowserScript.operator == None))
...
@mythic.route(mythic.config['API_BASE'] + "/browser_scripts/export/current_operation", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
