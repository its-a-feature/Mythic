...
async def get_all_mitre_attack_ids_by_task(request, user):
    ...
    try:
        ...
        return json({'status': 'error', 'error': "Must have an operation set as your current operation"})
    query = await db_model.attack_query()
    attack_entries = await db_objects.execute(query)
    matrix = {}
    for entry in attack_entries:
        ...
        for t in tactics:
            ...
            mappings = await db_objects.execute(query.where(
...
@mythic.route(mythic.config['API_BASE'] + "/mitreattack/task/<tid:int>/attack/<tnum:string>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_task_attack_mapping(request, user, tid, tnum):
    ...
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot remove MITRE from tasks'})
    try:
        query = await db_model.task_query()
