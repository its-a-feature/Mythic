...
@mythic.route(mythic.config['API_BASE'] + "/files/download/<id:string>", methods=['GET'])
    ...
    if 'full_path' in data and data['full_path'] is not None and data['full_path'] != "":
        ...
        if file_meta.task is None or file_meta.task != task:
            ...
            await db_objects.create(db_model.FileMeta, task=task, total_chunks=file_meta.total_chunks,
                                    ...
                                    delete_after_fetch=False, deleted=False,
                                    operator=task.operator, host=file_meta.host)
        else:
            ...
            if file_meta.full_remote_path is None or file_meta.full_remote_path == "":
                ...
                try:
                    fb_object = await db_objects.get(query,
...
async def create_filemeta_in_database_func(data):
    ...
    try:
        ...
        filemeta = await db_objects.create(FileMeta, total_chunks=data['total_chunks'], task=task, operation=operation,
                                           path="", operator=task.operator, full_remote_path=data['full_path'],
                                           delete_after_fetch=False, is_screenshot=is_screenshot, filename=filename.name,
                                           is_download_from_agent=True, host=data['host'])
        # check and see if there's a filebrowserobj that matches our full path
        query = await db_model.filebrowserobj_query()
        try:
            if not is_screenshot:
                fb_object = await db_objects.get(query, full_path=filemeta.full_remote_path.encode('unicode-escape'), host=data['host'].encode('unicode-escape'))
...
async def create_filemeta_in_database_manual(request, user):
    ...
    file_meta = await db_objects.create(FileMeta, total_chunks=1, operation=operation, path="",
                                        complete=True, chunks_received=1, operator=operator, delete_after_fetch=False,
                                        filename=filename)
    ...
    await db_objects.update(file_meta)
    await db_objects.create(db_model.OperationEventLog, operator=operator, operation=operation,
                            message="Apfell: {} hosted with UID {}".format(filename, file_meta.agent_file_id))
...
async def list_all_screencaptures_per_operation(request, user):
    ...
    if user['current_operation'] != "":
        ...
        operation = await db_objects.get(query, name=user['current_operation'])
        query = await db_model.filemeta_query()
        screencaptures = await db_objects.execute(query.where(
...
@mythic.route(mythic.config['API_BASE'] + "/files/screencaptures/bycallback/<id:int>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def list_all_screencaptures_per_callback(request, user, id):
    ...
    if callback.operation.name in user['operations']:
        query = await db_model.filemeta_query()
        screencaptures = await db_objects.prefetch(query.where(
            (FileMeta.operation == callback.operation) & (FileMeta.is_screenshot == True)
        ))
        for s in screencaptures:
...
@mythic.route(mythic.config['API_BASE'] + "/files/screencaptures/<id:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
async def download_zipped_files(request, user):
    ...
    try:
        ...
        for file_id in data['files']:
            try:
                cur_file = await db_objects.get(query, agent_file_id=file_id, operation=operation, deleted=False)
        ...
        file_meta = await db_objects.create(FileMeta, total_chunks=1, operation=operation,
