...
async def get_all_process_lists(request, user):
    ...
    process_lists = await db_objects.execute(query.where(db_model.ProcessList.operation == operation))
    return json({'status': 'success', 'process_lists': [l.to_json() for l in process_lists]})


@mythic.route(mythic.config['API_BASE'] + "/process_list/<pid:int>/<host:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def get_a_process_list(request, user, pid, host):
    ...
    if pid > 0:
        try:
            process_list = await db_objects.get(query.where(
                (db_model.ProcessList.operation == operation) & (db_model.ProcessList.id == pid) &
        ...
        # get the latest one
        latest = await db_objects.execute(query.where(
            (db_model.ProcessList.operation == operation) & (db_model.ProcessList.host == host)
        ).order_by(-db_model.ProcessList.timestamp).limit(1))
    ...
    try:
        ...
        return json({'status': 'error', 'error': 'failed to parse process list data as JSON'})
    try:
...
async def get_adjacent_process_list(request, user):
    ...
    if 'adjacent' not in data or data['adjacent'] not in ['next', 'prev']:
        return json({'status': 'error', 'error': 'must specify "adjacent" with a value of "next" or "prev"'})
    if 'host' not in data:
        return json({'status': 'error', 'error': '"host" is required'})
    if data['adjacent'] == 'prev':
        try:
            process_list = await db_objects.execute(query.where(
                (db_model.ProcessList.operation == operation) & (db_model.ProcessList.id < data['pid']) &
                (db_model.ProcessList.host == data['host'])
            ).order_by(-db_model.ProcessList.id).limit(1))
            process_list = list(process_list)
            if len(process_list) != 0:
                process_list = process_list[0]
            else:
                return json({'status': 'error', 'error': 'No earlier process lists for this host'})
        except Exception as e:
            return json({'status': 'error', 'error': 'No earlier process lists for this host'})
    else:
        # get the latest one
    ...
    try:
        ...
        return json({'status': 'error', 'error': 'Failed to parse process list as JSON'})
    try:
