...
@mythic.route(mythic.config['API_BASE'] + "/payloads/current_operation", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/payloads/current_operation/bytypes", methods=['POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def get_all_payloads_current_operation(request, user):
    if user['auth'] not in ['access_token', 'apitoken']:
    ...
    if user['current_operation'] != "":
        ...
        if 'ptypes' not in data:
            return json({'status': 'error', 'error': 'must specify a list of payload types'})
        for p in data['ptypes']:
            try:
                ...
                payloads = await db_objects.execute(query.where(
                    (Payload.operation == operation) & (Payload.deleted == False) & (Payload.payload_type == ptype)
                    & (Payload.auto_generated == False) & (Payload.build_phase == "success")
                ))
...
@mythic.route(mythic.config['API_BASE'] + "/payloads/<puuid:string>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_payload(request, puuid, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot remove payload'})
        query = await db_model.operation_query()
        operation = await db_objects.get(query, name=user['current_operation'])
...
async def remove_payload_func(uuid, operation):
    ...
    try:
        ...
        file_metas = await db_objects.execute(query.where(FileMeta.path == payload.file_id.path))
...
async def remove_multiple_payload(request, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot remove payloads'})
        query = await db_model.operation_query()
        operation = await db_objects.get(query, name=user['current_operation'])
        data = request.json
        errors = {}
...
async def register_new_payload_func(data, user):
    ...
    filename = data['location'] if 'location' in data else uuid
    # Register payload
    if 'build_container' not in data:
        data['build_container'] = payload_type.ptype
    if not payload_type.wrapper:
        file_meta = await db_objects.create(db_model.FileMeta, operation=operation, operator=operator, total_chunks=1,
                                            is_payload=True, complete=True, chunks_received=1, delete_after_fetch=False,
                                            filename=filename, path="./app/files/{}".format(uuid))
        payload = await db_objects.create(Payload, operator=operator, payload_type=payload_type, tag=tag,
                                          uuid=uuid, operation=operation, build_container=data['build_container'],
                                          file_id=file_meta)
        delete_elements.append(payload)
        delete_elements.append(file_meta)
        await db_objects.create(db_model.OperationEventLog, operation=operation,
                                message="New payload {} from {} with UUID {} and tag: {}".format(payload_type.ptype,
                                                                                                 operator.username,
                                                                                                    payload.uuid,
                                                                                                    payload.tag))
        ...
        for p in data['c2_profiles']:
            try:
                ...
                if c2_profile.container_running:
                    await send_c2_rabbitmq_message(c2_profile.name, "start", "", user['username'])
            except Exception as e:
                print(str(sys.exc_info()[-1].tb_lineno) + " " + str(e))
                for e in delete_elements:
                    await db_objects.delete(e)
            ...
            for param in db_c2_profile_parameters:
                ...
                try:
                    c2p = await db_objects.create(C2ProfileParametersInstance, c2_profile_parameters=param,
                                                  value=p['c2_profile_parameters'][param.name], payload=payload,
            ...
            try:
                payload_c2 = await db_objects.create(db_model.PayloadC2Profiles, payload=payload,
                                                     c2_profile=c2_profile)
        ...
        payload = await db_objects.create(Payload, operator=operator, payload_type=payload_type,
                                       tag=tag, build_container=data['build_container'], file_id=file_meta,
    ...
    if data['build_parameters']:
        query = await db_model.buildparameter_query()
        for t in data['build_parameters']:
            build_param = await db_objects.get(query, name=t['name'], payload_type=payload.payload_type, deleted=False)
            if 'value' not in t:
                t['value'] = None
            await db_objects.create(db_model.BuildParameterInstance, build_parameter=build_param,
                                    payload=payload, parameter=t['value'])
    try:
...
async def write_payload(uuid, user, data):
    ...
    build_params = await db_objects.execute(bp_query.where(db_model.BuildParameterInstance.payload == payload))
    ...
    try:
        if payload.wrapped_payload is not None:
            if os.path.exists(payload.wrapped_payload.file_id.path):
                wrapped_payload = base64.b64encode(open(payload.wrapped_payload.file_id.path, "rb").read()).decode()
            else:
...
@mythic.route(mythic.config['API_BASE'] + "/payloads/download/<uuid:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/payloads/bytype/<ptype:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def get_payloads_by_type(request, ptype, user):
    ...
    query = await db_model.payload_query()
    payloads = await db_objects.execute(query.where(
        (Payload.operation == operation) & (Payload.payload_type == payloadtype) & (Payload.build_phase == "success")
    ))
...
async def get_payload_config(payload):
    ...
    commands = [{"cmd": c.command.cmd, "version": c.version, "mythic_version": c.command.version} for c in
                payloadcommands]
