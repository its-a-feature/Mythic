...
from app.database_models.model import C2Profile, PayloadTypeC2Profile, C2ProfileParameters, C2ProfileParametersInstance
...
async def get_all_c2profiles(request, user):
    ...
    for p in profiles:
        if p.c2_profile.name in inter:
            inter[p.c2_profile.name]['ptype'].append({"ptype": p.payload_type.ptype, "supported_os": p.payload_type.supported_os})
    for k in inter.keys():
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/type/<info:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/start", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def start_c2profile(request, info, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot start c2 profiles'})
        query = await db_model.c2profile_query()
...
# Start running a profile's server side code
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/stop", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/status", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def status_c2profile(request, info, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot query c2 profiles'})
        query = await db_model.c2profile_query()
        profile = await db_objects.get(query, id=info)
    except Exception as e:
        print(e)
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/files/container_config_download", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def download_container_file_for_c2profiles(request, info, user):
    ...
    try:
        ...
        return json({'status': 'error', 'error': 'failed to find C2 Profile'})
    try:
        status = await send_c2_rabbitmq_message(profile.name, "get_config", "", user['username'])
        return json(status)
    except Exception as e:
        return json({'status': 'error', 'error': 'failed finding the file: ' + str(e)})


@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/files/container_config_upload", methods=['POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def upload_container_file_for_c2profiles(request, info, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot modify c2 profile files'})
        query = await db_model.c2profile_query()
        profile = await db_objects.get(query, id=info)
    except Exception as e:
        print(e)
    ...
    try:
        data = request.json
        status = await send_c2_rabbitmq_message(profile.name, "writefile",
                                 js.dumps({"file_path": "config.json", "data": data['code']}), user['username'])
        return json(status)
    except Exception as e:
        return json({'status': 'error', 'error': 'failed finding the file: ' + str(e)})
...
async def delete_c2profile(request, info, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot delete c2 profiles'})
        query = await db_model.c2profile_query()
        profile = await db_objects.get(query, id=info)
        query = await db_model.payloadtypec2profile_query()
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/parameters/", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
    ...
    try:
        ...
        for p in parameters:
            ...
            if p_json['randomize']:
                ...
                p_json['default_value'] = await generate_random_format_string(p_json['format_string'])
            param_list.append(p_json)
        return json({'status': 'success', 'c2profileparameters': param_list})
    except Exception as e:
        print(e)
        return json({'status': 'error', 'error': 'failed to get c2 profile parameters, you might need to select a c2 profile first'})
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<profile:int>/files/sync", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def sync_container_file_for_c2_profile(request, profile, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot sync containers'})
        query = await db_model.c2profile_query()
        ...
        return json({'status': 'error', 'error': 'failed to find C2 Profile'})
    try:
        status = await send_c2_rabbitmq_message(c2profile.name, "sync_classes", "", user['username'])
        return json(status)
    except Exception as e:
        return json({'status': 'error', 'error': 'failed finding the file: ' + str(e)})
...
async def update_c2_profile(request, profile, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot modify c2 profiles'})
        query = await db_model.c2profile_query()
        c2profile = await db_objects.get(query, id=profile)
    except Exception as e:
        print(e)
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/parameter_instances/", methods=['POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def save_c2profile_parameter_value_instance(request, info, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot save c2 profile instances'})
        query = await db_model.operation_query()
        operation = await db_objects.get(query, name=user['current_operation'])
        query = await db_model.c2profile_query()
        profile = await db_objects.get(query, id=info)
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/parameter_instances/", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/<info:int>/parameter_instances/", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/c2profiles/parameter_instances/<instance_name:string>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def delete_c2profile_parameter_value_instance(request, instance_name, user):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot delete c2 profile instances'})
...
async def import_c2_profile_func(data, operator):
    try:
        ...
        profile = await db_objects.create(C2Profile, name=data['name'],
                                          description=data['description'], author=data['author'], mythic_encrypts=data['mythic_encrypts'],
                                          is_p2p=data['is_p2p'], is_server_routed=data['is_server_routed'])
        #print("Created new c2 profile: {}".format(data['name']))
    query = await db_model.c2profileparameters_query()
    curr_parameters = await db_objects.execute(query.where(db_model.C2ProfileParameters.c2_profile == profile))
    ...
    for param in data['params']:
        try:
            ...
            await db_objects.create(C2ProfileParameters, c2_profile=profile, name=param['name'], description=param['description'],
                                    default_value=param['default_value'], randomize=param['randomize'], format_string=param['format_string'],
                                    required=param['required'], parameter_type=param['parameter_type'],
