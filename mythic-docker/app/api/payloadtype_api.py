...
from app.database_models.model import PayloadType, Command, CommandParameters, ATTACKCommand, \
...
async def get_all_payloadtypes(request, user):
    ...
    for pt in payloads:
        ...
        build_params = await db_objects.execute(build_query.where((db_model.BuildParameter.payload_type == pt) &
                                                                  (db_model.BuildParameter.deleted == False)))
        pt_json = pt.to_json()
        pt_json['build_parameters'] = [bp.to_json() for bp in build_params]
        if pt.wrapper:
            wrapped_types = await db_objects.execute(wrapquery.where(
                db_model.WrappedPayloadTypes.wrapper == pt
            ))
            wrappers.append({**pt_json,
                             'wrapped': [w.to_json() for w in wrapped_types]})
        else:
...
async def get_one_payloadtype(request, user, ptype):
    ...
    try:
        ...
        build_parameters = await db_objects.execute(query.where(
            (db_model.BuildParameter.payload_type == payloadtype) & (db_model.BuildParameter.deleted == False)))
    except Exception as e:
        return json({'status': 'error', 'error': 'failed to find payload type'})
    return json({'status': 'success', **payloadtype.to_json(), 'build_parameters': [bp.to_json() for bp in build_parameters]})
...
# payloadtypes aren't inherent to an operation
@mythic.route(mythic.config['API_BASE'] + "/payloadtypes/<ptype:int>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def delete_one_payloadtype(request, user, ptype):
    ...
    if user['admin']:
        ...
        try:
            ...
            payloadtype.ptype = str(uuid.uuid1()) + " ( " + payloadtype.ptype + " )"
            await db_objects.update(payloadtype)
        ...
        return json({'status': 'error', 'error': 'you must be admin to delete payload types'})


# get all the commands associated with a specific payload_type
@mythic.route(mythic.config['API_BASE'] + "/payloadtypes/<ptype:int>/commands", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def get_commands_for_payloadtype(request, user, ptype):
    ...
    for cmd in commands:
        ...
        all_commands.append(
            {**cmd.to_json(), "params": [p.to_json() for p in params]})
    status = {'status': 'success'}
    return json({**status, 'commands': all_commands})


@mythic.route(mythic.config['API_BASE'] + "/payloadtypes/<ptype:int>/files/sync", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def sync_container_file_for_payload_type(request, ptype, user):
    ...
    try:
        status = await send_pt_rabbitmq_message(payload_type.ptype, "sync_classes", "", user['username'])
        return json(status)
    except Exception as e:
    ...
    try:
        ...
        if ptype['wrapper']:
            try:
                ...
                payload_type.supports_dynamic_loading = ptype['supports_dynamic_loading']
            except Exception as e:
                payload_type = await db_objects.create(PayloadType, ptype=ptype['ptype'], wrapper=True,
            ...
            current_wrapped = await db_objects.execute(wrapper_query.where(db_model.WrappedPayloadTypes.wrapper == payload_type))
            ...
            for ptw in ptype['wrapped']:
                try:
                    ...
                    await db_objects.create(db_model.WrappedPayloadTypes, wrapper=payload_type, wrapped=wrapped)
                except Exception as e:
                    print(e)
                    pass
        else:
            try:
                ...
                payload_type = await db_objects.create(PayloadType, ptype=ptype['ptype'], wrapper=False,
                                                       supported_os=ptype['supported_os'],
                                                       file_extension=ptype['file_extension'],
                                                       author=ptype['author'],
                                                       note=ptype['note'],
        ...
        try:
            ...
            if 'build_parameters' in ptype:
                query = await db_model.buildparameter_query()
                current_params = await db_objects.execute(query.where(
                    (db_model.BuildParameter.payload_type == payload_type) & (db_model.BuildParameter.deleted == False)))
                build_param_dict = {b.name: b for b in current_params}
                for bp in ptype['build_parameters']:
                    if bp['name'] == "":
                        continue
                    try:
                        buildparam = await db_objects.get(query, payload_type=payload_type, name=bp['name'], deleted=False)
                        ...
                        await db_objects.create(db_model.BuildParameter, payload_type=payload_type, name=bp['name'],
                                                parameter_type=bp['parameter_type'], description=bp['description'],
                                                required=bp['required'], verifier_regex=bp['verifier_regex'],
            ...
            if 'support_scripts' in ptype:
                ...
                for support_script in ptype['support_scripts']:
                    try:
                        ...
                        script = await db_objects.get(browserscriptquery, name=support_script['name'], payload_type=payload_type)
                        ...
                        await db_objects.create(db_model.BrowserScript, name=support_script['name'], script=support_script['script'],
                                                container_version=support_script['script'], payload_type=payload_type,
                                                author=support_script['author'], container_version_author=support_script['author'])
                    # now loop through all users
                    operators = await db_objects.execute(operator_query.where(db_model.Operator.deleted == False))
                    for op in operators:
                        try:
                            # first update the base case,then loop through operators
                            script = await db_objects.get(browserscriptquery, name=support_script['name'],
                                                          payload_type=payload_type, operator=op)
                            script.container_version = support_script['script']
                            script.container_version_author = support_script['author']
                            ...
                            await db_objects.create(db_model.BrowserScript, name=support_script['name'],
                                                    script=support_script['script'],
                                                    container_version=support_script['script'],
                                                    operator=op, payload_type=payload_type,
            ...
            if 'c2_profiles' in ptype:
                query = await db_model.payloadtypec2profile_query()
                current_c2 = await db_objects.execute(query.where(db_model.PayloadTypeC2Profile.payload_type == payload_type))
...
async def import_command_func(payload_type, operator, command_list):
    query = await db_model.command_query()
    current_commands = await db_objects.execute(query.where(
        (db_model.Command.payload_type == payload_type) & (db_model.Command.deleted == False)))
    for command in current_commands:
        ...
        if command.cmd in command_list:
            ...
            if 'is_upload_file' not in cmd:
                ...
                try:
                    rf_command = await db_objects.get(
                        query.where((Command.is_upload_file == True) & (Command.payload_type == payload_type) & (
                                    Command.deleted == False)))
                    # one is already set, so set it to false
            ...
            for param in cmd['parameters']:
                try:
                    ...
                    if 'supported_agents' in param and param['supported_agents'] is not None:
            ...
            if 'browser_script' in cmd:
                ...
                try:
                    # first update the base case,then loop through operators
                    script = await db_objects.get(browserscriptquery, name=cmd['browser_script']['name'],
                                                  payload_type=payload_type, command=command)
                    ...
                    await db_objects.create(db_model.BrowserScript, name=cmd['browser_script']['name'],
                                            script=cmd['browser_script']['script'],
                                            container_version=cmd['browser_script']['script'],
                                            payload_type=payload_type,
                                            command=command, author=cmd['browser_script']['author'],
                ...
                for op in operators:
                    try:
                        # first update the base case,then loop through operators
                        script = await db_objects.get(browserscriptquery, name=cmd['browser_script']['name'],
                                                      payload_type=payload_type, operator=op, command=command)
                        script.container_version = cmd['browser_script']['script']
                        script.container_version_author = cmd['browser_script']['author']
                        if not script.user_modified:
                            script.script = cmd['browser_script']['script']
    ...
    for cmd_name, cmd in command_list.items():
        ...
        if 'is_upload_file' not in cmd:
            ...
            try:
                rf_command = await db_objects.get(
                    query.where((Command.is_upload_file == True) & (Command.payload_type == payload_type) & (Command.deleted == False)))
                # one is already set, so set it to false
                rf_command.is_upload_file = False
                await db_objects.update(rf_command)
            except Exception as e:
        ...
        if 'browser_script' in cmd:
            ...
            try:
                ...
                await db_objects.create(db_model.BrowserScript, name=cmd['browser_script']['name'],
                                        script=cmd['browser_script']['script'],
                                        container_version=cmd['browser_script']['script'], payload_type=payload_type,
                                        command=command, author=cmd['browser_script']['author'],
                                        container_version_author=cmd['browser_script']['author'])
            # now loop through all users
            operator_query = await db_model.operator_query()
