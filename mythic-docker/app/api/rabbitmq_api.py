    ...
    with message.process():
        ...
        if pieces[4] == "sync_classes":
            if pieces[5] == "":
                ...
                operator = await db_objects.get(query, username=base64.b64decode(pieces[5]).decode())
            from app.api.c2profiles_api import import_c2_profile_func
            status = await import_c2_profile_func(json.loads(message.body.decode()), operator)
            ...
            if status['status'] == "success":
                if status['name'] != pieces[2]:
                    logger.warning("Imported C2 {} from container {}".format(status['name'], pieces[2]))
                ...
                pts = await db_objects.execute(query.where(db_model.PayloadType.wrapper == False))
                ...
                logger.error(
                    "Failed to update database from docker code for {}: {}".format(pieces[2], status['error']))
                for o in operations:
                    await db_objects.create(db_model.OperationEventLog, operator=operator, level="info",
                                            operation=o,
...
async def rabbit_pt_rpc_callback(exchange: aio_pika.Exchange, message: aio_pika.IncomingMessage):
    with message.process():
        request = json.loads(message.body.decode())
        if 'task_id' not in request:
            response = json.dumps({"status": "error", "error": "Missing task_id"}).encode()
            ...
            response = json.dumps({"status": "error", "error": "Missing action"}).encode()
        try:
            ...
            error = "Exception trying to send message back to container for rpc! " + str(e)
            error += "\nResponse: {}\nCorrelation_id: {}\n RoutingKey: {}".format(
            ...
            task.completed = True
            await db_objects.update(task)
            await db_objects.create(db_model.Response, task=task, response=error.encode('unicode-escape'))
...
async def register_file(request):
    ...
    try:
        task_query = await db_model.task_query()
        task = await db_objects.get(task_query, id=request['task_id'])
        filename = request['saved_file_name'] if 'saved_file_name' in request else str(uuid.uuid4())
        ...
        new_file_meta = await db_objects.create(db_model.FileMeta, total_chunks=1, chunks_received=1, complete=True,
                                                path=str(path), operation=task.callback.operation, operator=task.operator,
                                                full_remote_path=request['remote_path'], md5=md5, sha1=sha1, task=task,
                                                delete_after_fetch=request["delete_after_fetch"], filename=filename,
                                                is_screenshot=request['is_screenshot'], is_download_from_agent=request['is_download'])
...
async def get_file_by_name(request):
    try:
        ...
        files = await db_objects.execute(file_query.where(
            (db_model.FileMeta.deleted == False) & (db_model.FileMeta.filename == request['filename']) &
        ...
        if file is None:
            ...
            if os.path.exists(file.path):
                file_json['contents'] = base64.b64encode(open(file.path, 'rb').read()).decode()
...
async def get_payload_by_uuid(request):
    try:
        ...
        if os.path.exists(payload.file_id.path):
            payload_json['contents'] = base64.b64encode(open(payload.file_id.path, 'rb').read()).decode()
...
async def build_payload_from_template(request):
    ...
    try:
        ...
        if 'destination_host' in request and request['destination_host'] != "" and request['destination_host'] is not None and request['destination_host'] not in ['localhost', '127.0.0.1', '::1']:
        ...
        data = {"payload_type": template.payload_type.ptype, "c2_profiles": [],
                "commands": [],  "tag": "Autogenerated from task {} on callback {}".format(str(task.id), str(task.callback.id)),
        ...
        for c2p in c2profiles:
            ...
            for p in c2_profile_params:
                if p.c2_profile_parameters.randomize:
                    params[p.c2_profile_parameters.name] = await generate_random_format_string(p.c2_profile_parameters.format_string)
                else:
        ...
        data['location'] = "Task" + str(task.id) + "Copy_" + template.file_id.filename
        #print(data)
        ...
        if rsp['status'] == 'success':
            ...
            if create_rsp['status'] != "success":
                ...
                return {'status': 'error', 'error': 'Failed to send build message to container: ' + create_rsp['error']}
            else:
                task.status = "building..."
                task.timestamp = datetime.datetime.utcnow()
                await db_objects.update(task)
            ...
            return {'status':  'error', 'error': 'Failed to register new payload: ' + rsp['error']}
    except Exception as e:
        print(str(sys.exc_info()[-1].tb_lineno) + " " + str(traceback.format_exc()))
        return {'status': 'error', 'error': 'Failed to build payload: ' + str(traceback.format_exc())}
    ...
    try:
        ...
        if enc_key['type'] is not None:
            if enc_key['type'] == "AES256":
                enc_message = await encrypt_AES256(base64.b64decode(request['data']),
                                                   base64.b64decode(enc_key['enc_key']))
        else:
            enc_message = request['data']
        if 'with_uuid' in request and request['with_uuid']:
            message = base64.b64encode(task.callback.agent_callback_id.encode() + enc_message).decode()
        else:
...
async def decrypt_bytes(request):
    ...
    try:
        ...
        if 'with_uuid' in request and request['with_uuid']:
            ...
            callback = await db_objects.get(callback_query, agent_callback_id=message[:36])
            dec_message = await decrypt_AES256(message[36:],
                                               base64.b64decode(callback.decryption_key))
        else:
            dec_message = await decrypt_AES256(base64.b64decode(request['data']),
                                               base64.b64decode(task.callback.decryption_key))
        return {'status': 'success', 'response': {
            "data": base64.b64encode(dec_message).decode()
        }}
    except Exception as e:
        return {"status": 'error', 'error': str(e)}
...
async def user_output(request):
    try:
        task_query = await db_model.task_query()
        task = await db_objects.get(task_query, id=request['task_id'])
        resp = await db_objects.create(db_model.Response, task=task, response=request['user_output'].encode('unicode-escape'))
...
async def task_update_callback(request):
    try:
        ...
        status = await update_callback(request['callback_info'], task.callback.agent_callback_id)
        return status
    except Exception as e:
...
async def register_artifact(request):
    ...
    try:
        ...
        await db_objects.create(db_model.TaskArtifact, task=task, artifact_instance=request['artifact_instance'],
                                artifact=artifact, host=request['host'], operation=task.callback.operation)
        return {'status': 'success'}
    except Exception as e:
        return {'status': 'error', 'error': 'failed to create task artifact: ' + str(e)}


async def rabbit_c2_rpc_callback(exchange: aio_pika.Exchange, message: aio_pika.IncomingMessage):
    with message.process():
        print(message)
        request = json.loads(message.body.decode())
        ...
        try:
            ...
            operations = await db_objects.execute(operation_query.where(db_model.Operation.complete == False))
            for o in operations:
                await db_objects.create(db_model.OperationEventLog, level="error",
                                        operation=o, message="Failed to process C2 RPC: {}".format(str(e)))
...
async def get_tasking(request):
    try:
        ...
        decrypted = {
            "action": "get_tasking",
            "tasking_size": request['tasking_size']
        }
        ...
        if enc_key['type'] is not None:
            if enc_key['type'] == "AES256":
                enc_message = await encrypt_AES256(json.dumps(response_data).encode(),
                                                   base64.b64decode(callback.encryption_key))
            else:
                enc_message = json.dumps(response_data).encode()
            enc_message = base64.b64encode(callback.agent_callback_id.encode() + enc_message).decode()
        else:
            enc_message = base64.b64encode((callback.agent_callback_id + json.dumps(response_data)).encode()).decode()
...
async def add_event_message(request):
    try:
        ...
        operations = await db_objects.execute(operation_query.where(db_model.Operation.complete == False))
        for o in operations:
            await db_objects.create(db_model.OperationEventLog, level=request['level'],
                                    operation=o, message=request['message'])
        return {'status': 'success'}
    except Exception as e:
        return {'status': 'error', 'error': str(e)}
...
async def connect_and_consume_rpc():
    ...
    while connection is None:
        try:
            ...
            try:
                task = queue.consume(partial(rabbit_pt_rpc_callback, channel.default_exchange))
                result = await asyncio.wait_for(task, None)
            except Exception as e:
                logger.exception("Exception in connect_and_consume_rpc .consume: {}".format(str(e)))
                # print("Exception in connect_and_consume .consume: {}".format(str(e)))
        except (ConnectionError, ConnectionRefusedError) as c:
            logger.error("Connection to rabbitmq failed, trying again...")
        except Exception as e:
            logger.exception("Exception in connect_and_consume_rpc connect: {}".format(str(e)))
            # print("Exception in connect_and_consume connect: {}".format(str(e)))
        await asyncio.sleep(2)
