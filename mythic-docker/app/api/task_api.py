...
from app.database_models.model import Callback, Task, FileMeta, Response, LoadedCommands, ATTACKCommand, ATTACKTask, TaskArtifact, OperatorOperation, Payload, Command, C2ProfileParametersInstance
...
async def search_tasks(request, user):
    ...
    try:
        ...
        return json({'status': 'error', 'error': 'Cannot get that operation or operator'})
    try:
        query = await db_model.task_query()
        if data['type'] == "params":
            if 'operator' in data:
                ...
                ).switch(Callback).where(Callback.operation == operation).distinct())
            else:
            ...
            if 'operator' in data:
                count = await db_objects.count(query.where(Command.cmd.regexp(data['search'])).switch(Callback).where(Callback.operation == operation).switch(Task).where(Task.operator == data['operator']).distinct())
            else:
                count = await db_objects.count(query
                                           .where(Command.cmd.regexp(data['search'])).switch(Callback).where(Callback.operation == operation))
        else:
            if 'operator' in data:
                count = await db_objects.count(query.where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(Callback.operation == operation).switch(Task).where(Task.comment_operator == data['operator']).distinct())
            else:
                count = await db_objects.count(query
                                           .where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(Callback.operation == operation).distinct())
        if 'page' not in data:
            data['page'] = 1
            data['size'] = count
            if data['type'] == "params":
                if 'operator' in data:
                    tasks = await db_objects.prefetch(query.where(
                        ( (Task.params.regexp(data['search'])) | (Task.original_params.regexp(data['search'])) ) & (Task.operator == data['operator'])
                    ).switch(Callback).where(Callback.operation == operation).order_by(Task.id).paginate(data['page'], data['size']).distinct())
                else:
                ...
                if 'operator' in data:
                    tasks = await db_objects.prefetch(query.switch(Command).where(Command.cmd.regexp(data['search'])).switch(Callback).where(
                        Callback.operation == operation).switch(Task).where(Task.operator == data['operator']).order_by(Task.id).paginate(data['page'], data['size']).distinct())
                else:
                    tasks = await db_objects.prefetch(query.switch(Command)
                                           .where(Command.cmd.regexp(data['search'])).switch(Callback).where(Callback.operation == operation).order_by(Task.id).paginate(data['page'], data['size']).distinct())
            else:
                if 'operator' in data:
                    tasks = await db_objects.prefetch(
                        query.where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(
                            Callback.operation == operation).switch(Task).where(
                            Task.comment_operator == data['operator']).order_by(Task.id).paginate(data['page'], data['size']).distinct())
                else:
                    tasks = await db_objects.prefetch(query
                                                      .where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(
                        Callback.operation == operation).order_by(Task.id).paginate(data['page'], data['size']).distinct())
        else:
            ...
            if data['type'] == "params":
                ...
                if 'operator' in data:
                    tasks = await db_objects.prefetch(query.where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(
                        Callback.operation == operation).switch(Task).where(Task.comment_operator == data['operator']).order_by(Task.id).paginate(data['page'], data['size']).distinct())
                else:
                    tasks = await db_objects.prefetch(query
                                                  .where((Task.comment.regexp(data['search']))&(Task.comment != "")).switch(Callback).where(Callback.operation == operation).order_by(Task.id).paginate(data['page'], data['size']).distinct())
        output = []
        for t in tasks:
    ...
    try:
        ...
        if not callback.active:
            ...
            c2profiles = await db_objects.execute(c2_query.where(
                db_model.CallbackC2Profiles.callback == callback
            ))
            for c2 in c2profiles:
                if not c2.c2_profile.is_p2p:
                    try:
                        edge = await db_objects.get(db_model.CallbackGraphEdge,
                                                    source=callback, destination=callback, c2_profile=c2.c2_profile,
                                                    direction=1, end_timestamp=None, operation=callback.operation)
                    except Exception as d:
                        print(d)
                        edge = await db_objects.create(db_model.CallbackGraphEdge,
                                                       source=callback, destination=callback, c2_profile=c2.c2_profile,
                                                       direction=1, end_timestamp=None, operation=callback.operation)
                        from app.api.callback_api import add_non_directed_graphs, add_directed_graphs
        ...
        if not callback.operation.complete:
            ...
            await db_objects.create(db_model.OperationEventLog, operation=callback.operation, level="warning",
                                    message="Callback {} still checking in".format(callback.id))
            tasks = []
    except Exception as e:
        logger.exception("Error in getting tasking for : " + str(callback.id) + ", " + str(e))
        tasks = []
...
async def add_task_to_callback_func(data, cid, user, op, operation, cb):
    try:
        ...
        if payload_type.container_running:
            task = await db_objects.create(Task, callback=cb, operator=op, command=cmd,
            ...
            rabbit_message['task']['callback']['build_parameters'] = payload_info['build_parameters']
...
async def add_all_payload_info(payload):
    ...
    if payload.uuid in cached_payload_info:
        rabbit_message['build_parameters'] = cached_payload_info[payload.uuid]['build_parameters']
...
@mythic.route(mythic.config['API_BASE'] + "/tasks/callback/<cid:int>/notcompleted", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/tasks/<tid:int>/raw_output", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def get_one_task_and_responses_as_raw_output(request, tid, user):
    ...
    try:
        ...
        if task.callback.operation.name in user['operations']:
            query = await db_model.response_query()
            responses = await db_objects.execute(query.where(Response.task == task).order_by(Response.id))
            output = ''.join([bytes(r.response).decode('unicode-escape', errors='backslashreplace').encode('utf-8', errors="backslashreplace").decode() for r in responses])
...
async def add_comment_to_task(request, tid, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot comment on tasks'})
        query = await db_model.task_query()
        task = await db_objects.prefetch(query.where(Task.id == tid), Command.select())
        task = list(task)[0]
        data = request.json
...
@mythic.route(mythic.config['API_BASE'] + "/tasks/comments/<tid:int>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_task_comment(request, tid, user):
    ...
    try:
        if user['view_mode'] == 'spectator':
            return json({'status': 'error', 'error': 'Spectators cannot remove comments on tasks'})
        query = await db_model.task_query()
        task = await db_objects.prefetch(query.where(Task.id == tid), Command.select())
        task = list(task)[0]
        query = await db_model.operator_query()
