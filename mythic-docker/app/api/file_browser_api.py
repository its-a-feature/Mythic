...
async def get_filebrowser_tree_for_operation(operation_name):
    ...
    try:
        query = await db_model.filebrowserobj_query()
        objs = await db_objects.execute(query.where(db_model.FileBrowserObj.operation == operation))
        output = {}
        roots = []
        sorted_obj = []
        for o in objs:
        ...
        for o in sorted_obj:
            for k,v in output[o['host']].items():
                if o['parent_path'].startswith(k):
                    #print("trying to add")
                    #print(o)
                    output[o['host']][k].create_node(o['name'], o['id'], parent=o['parent'], data=o)
        final_output = {}
        for k,v in output.items():
            final_output[k] = {"children": [], "data": {"host": k, "is_file": False, "name": k}}
...
async def store_response_into_filebrowserobj(operation, task, response):
    ...
    if parent is None and response['parent_path'] != "":
        return {'status': 'error', 'error': 'Failed to parse and handle file browser objects'}
    ...
    try:
        if response['name'] == "/" or len(response['parent_path']) > 0 and response['parent_path'][0] == "/":
        ...
        try:
            filebrowserobj = await db_objects.get(query, operation=operation, host=response['host'].encode('unicode-escape'),
                                                  name=response['name'].encode('unicode-escape'), is_file=response['is_file'],
                                                  parent=parent, parent_path=str(parent_path_str).encode('unicode-escape'))
            filebrowserobj.task = task
            filebrowserobj.permissions = js.dumps(response['permissions']).encode('unicode-escape')
            filebrowserobj.access_time = response['access_time'].encode('unicode-escape')
            filebrowserobj.modify_time = response['modify_time'].encode('unicode-escape')
            ...
            filebrowserobj = await db_objects.create(db_model.FileBrowserObj, task=task, operation=operation,
                                                 host=response['host'].encode('unicode-escape'), name=response['name'].encode('unicode-escape'),
                                                 ...
                                                 modify_time=response['modify_time'].encode('unicode-escape'), is_file=response['is_file'],
                                                 size=str(response['size']).encode('unicode-escape'), success=response['success'])
        if not filebrowserobj.is_file and 'files' in response and response['files'] is not None and len(response['files']) > 0:
            # iterate over the files and create their objects
            parent_path = parent_path.joinpath(response['name'])
            for f in response['files']:
                try:
                    newfileobj = await db_objects.get(query, operation=operation,
                    ...
                    newfileobj.permissions = js.dumps(f['permissions']).encode('unicode-escape')
                    ...
                    await db_objects.create(db_model.FileBrowserObj, task=task, operation=operation, host=response['host'].encode('unicode-escape'),
                                        parent=filebrowserobj, permissions=js.dumps(f['permissions']).encode('unicode-escape'), parent_path=str(parent_path).encode('unicode-escape'),
                                        access_time=f['access_time'].encode('unicode-escape'), modify_time=f['modify_time'].encode('unicode-escape'), size=str(f['size']).encode('unicode-escape'),
                                        is_file=f['is_file'], name=f['name'].encode('unicode-escape'),
...
async def create_and_check_parents(operation, task, response):
    ...
    try:
        ...
        for p in range(len(parent_path.parts)):
            ...
            try:
                parent = await db_objects.get(query, host=response['host'].encode('unicode-escape'), parent=parent_obj, name=name.encode('unicode-escape'),
                                              operation=operation)
            except Exception as e:
                ...
                parent = await db_objects.create(db_model.FileBrowserObj, task=task, operation=operation,
                                                 host=response['host'].encode('unicode-escape'), name=name.encode('unicode-escape'),
                                                 parent=parent_obj, parent_path=parent_path_name.encode('unicode-escape'),
                                                 full_path=full_path)
            parent_obj = parent
        return parent_obj
    except Exception as e:
...
async def search_filebrowsobj(request, user):
    ...
    try:
        ...
        count = await db_objects.count(query.where(
            ...
            (fn.encode(db_model.FileBrowserObj.host, 'escape').regexp(data['host'])) &
            (db_model.FileBrowserObj.comment.regexp(data['comment'])) &
            (fn.encode(db_model.FileBrowserObj.full_path, 'escape').regexp(data['path']))).distinct())
        if 'page' not in data:
            ...
            (fn.encode(db_model.FileBrowserObj.host, 'escape').regexp(data['host'])) &
            (db_model.FileBrowserObj.comment.regexp(data['comment'])) &
            (fn.encode(db_model.FileBrowserObj.full_path, 'escape').regexp(data['path']))).distinct().order_by(db_model.FileBrowserObj.full_path))
            data['page'] = 1
            data['size'] = count
        else:
            ...
            responses = await db_objects.execute(query.where(
                ...
                (fn.encode(db_model.FileBrowserObj.host, 'escape').regexp(data['host'])) &
                (db_model.FileBrowserObj.comment.regexp(data['comment'])) &
                (fn.encode(db_model.FileBrowserObj.full_path, 'escape').regexp(data['path']))).distinct().order_by(db_model.FileBrowserObj.full_path).paginate(data['page'], data['size']))
        output = []
        for r in responses:
            rjson = r.to_json()
            rjson['files'] = []
