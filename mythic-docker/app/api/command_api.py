...
@mythic.route(mythic.config['API_BASE'] + "/commands/<ptype:int>/check/<cmd:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/commands/<id:int>/parameters/", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/commands/<id:int>/mitreattack/", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/commands/<id:int>/mitreattack/<t_num:string>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_attack_mapping_for_command(request, user, id, t_num):
    ...
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot remove MITRE mappings'})
    try:
        query = await db_model.command_query()
        command = await db_objects.get(query, id=id)
...
@mythic.route(mythic.config['API_BASE'] + "/commands/<id:int>/mitreattack/<t_num:string>", methods=['POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def create_attack_mappings_for_command(request, user, id, t_num):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot add MITRE mappings'})
    try:
        query = await db_model.command_query()
        command = await db_objects.get(query, id=id)
        query = await db_model.attack_query()
...
@mythic.route(mythic.config['API_BASE'] + "/commands/<id:int>/mitreattack/<t_num:string>", methods=['PUT'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def adjust_attack_mappings_for_command(request, user, id, t_num):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot modify MITRE mappings'})
    data = request.json
    try:
        query = await db_model.command_query()
        command = await db_objects.get(query, id=id)
