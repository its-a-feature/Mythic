...
@mythic.route(mythic.config['API_BASE'] + "/credentials/current_operation", methods=['GET'])
@inject_user()
...
async def create_credential_func(operator, operation, data):
    ...
    if "realm" not in data or data['realm'] == "" or data['realm'] is None:
        data['realm'] = ""
    if "credential" not in data or data['credential'] == "" or data['credential'] is None:
        data['credential'] = ""
    ...
    if "task" not in data or data['task'] == "":
        try:
            ...
            query = await db_model.credential_query()
            cred = await db_objects.get(query, type=data['credential_type'], account=data['account'], deleted=False,
...
async def remove_credential(request, user, id):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot remove credentials'})
    if user['current_operation'] != "":
        try:
            query = await db_model.operation_query()
            operation = await db_objects.get(query, name=user['current_operation'])
...
async def modify_credential(request, user, id):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot modify credentials'})
    if user['current_operation'] != "":
        try:
            query = await db_model.operation_query()
            operation = await db_objects.get(query, name=user['current_operation'])
