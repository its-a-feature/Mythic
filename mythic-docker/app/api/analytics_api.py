...
@mythic.route(mythic.config['API_BASE'] + "/analytics/command_frequency", methods=['GET', 'POST'])
@inject_user()
...
@mythic.route(mythic.config['API_BASE'] + "/analytics/callback_analysis", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/analytics/artifact_overview", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def analytics_artifact_overview_api(request, user):
    ...
    manual_tasks = await db_objects.execute(task_query.where(
    ...
    for t in artifact_tasks:
        if t.task is not None:  # this was automatically reported by a task
            ...
            if artifact_name not in output['artifact_counts']:
                output['artifact_counts'][artifact_name] = {"agent_reported": 0, "manual": 0}
    ...
    for t in manual_tasks:
        if bytes(t.artifact.name).decode() not in output['artifact_counts']:
            output['artifact_counts'][bytes(t.artifact.name).decode()] = {"agent_reported": 0, "manual": 0}
