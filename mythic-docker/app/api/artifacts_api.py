...
async def create_artifact(request, user):
    ...
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot add base artifacts'})
    data = request.json
    if "name" not in data:
...
async def update_artifact(request, user, id):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot modify base artifacts'})
    data = request.json
    try:
        query = await db_model.artifact_query()
...
async def delete_artifact(request, user, id):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot remove base artifacts'})
    try:
        query = await db_model.artifact_query()
        artifact = await db_objects.get(query, id=id)
    except Exception as e:
...
@mythic.route(mythic.config['API_BASE'] + "/artifact_tasks/<page:int>/<size:int>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/artifact_tasks/<aid:int>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def remove_artifact_tasks(request, user, aid):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot remove task artifacts'})
    try:
        query = await db_model.taskartifact_query()
...
async def create_artifact_task_manually(request, user):
    if user['auth'] not in ['access_token', 'apitoken']:
        abort(status_code=403, message="Cannot access via Cookies. Use CLI or access via JS in browser")
    if user['view_mode'] == 'spectator':
        return json({'status': 'error', 'error': 'Spectators cannot create task artifacts'})
    # get all of the artifact tasks for the current operation
    try:
        query = await db_model.operation_query()
        operation = await db_objects.get(query, name=user['current_operation'])
