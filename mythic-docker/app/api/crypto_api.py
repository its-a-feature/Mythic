...
async def staging_rsa(decrypted_message_json, UUID):
    if "session_id" not in decrypted_message_json or "pub_key" not in decrypted_message_json:
        logger.exception("Failed to get \"session_id\" or \"pub_key\" from message in staging_rsa")
        return None, None
    ...
    temp_uuid = str(uuid4())
    try:
        stage_info = await db_objects.create(StagingInfo, session_id=decrypted_message_json['session_id'],
                                             session_key=session_key_encoded, payload_uuid=UUID,
    ...
    session_key_encoded = await crypt.encrypt_pub_key(data=base64.b64decode(session_key_encoded),
                                                      key=base64.b64decode(decrypted_message_json['pub_key']))
    response = {"uuid": temp_uuid, "session_key": base64.b64encode(session_key_encoded).decode(),
                         "action": "staging_rsa", "session_id": decrypted_message_json['session_id']}
...
async def staging_dh(decrypted_message_json, UUID):
    if "session_id" not in decrypted_message_json or "pub_key" not in decrypted_message_json:
        logger.exception("Failed to get \"session_id\" or \"pub_key\" from message in staging_dh")
        return None, None
    # generate random AES256 key
    dh = DiffieHellman()
    # print(dh.publicKey)
    ...
    try:
        ...
        return None, None
        # encrypt a nonce and the session_key_encoded with the message['PUB'] public key from the agent
    response = {"uuid": temp_uuid, "session_key": dh.publicKey,
                "action": "staging_dh", "session_id": decrypted_message_json['session_id']}
    # print("created response: " + js.dumps(response))
    for k in decrypted_message_json:
        if k not in ['session_id', 'pub_key', "action", 'delegates']:
            response[k] = decrypted_message_json[k]
