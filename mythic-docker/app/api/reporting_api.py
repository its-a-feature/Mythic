...
@mythic.route(mythic.config['API_BASE'] + "/reporting/full_timeline_pdf", methods=['GET', 'POST'])
@inject_user()
        ...
        try:
            ...
            if status['status'] == 'success':
                ...
                filemeta = await db_objects.create(FileMeta, total_chunks=1, operation=operation, path=save_path,
                                                   operator=operator, complete=True, filename="{} Timeline.pdf".format(user['current_operation']))
...
@mythic.route(mythic.config['API_BASE'] + "/reporting/full_timeline_json", methods=['GET', 'POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
    ...
    try:
        if user['current_operation'] != "":
            ...
            try:
                ...
                operator = await db_objects.get(query, username=user['username'])
                filemeta = await db_objects.create(FileMeta, total_chunks=1, operation=operation, path=save_path,
                                                   operator=operator, complete=True, filename="{} Full Timeline.json".format(user['current_operation']))
