    ...
    try:
        ...
        for o in operations:
            if user['admin'] or o.name in user['operations']:
                operatorsmap = await db_objects.execute(query.where(OperatorOperation.operation == o))
                ojson = o.to_json()
                ojson.pop("AESPSK", None)
                ojson['members'] = []
                for map in operatorsmap:
                    data = {"username": map.operator.username, "view_mode": map.view_mode}
        ...
        return json({"status": 'error', 'error': 'Failed to get operation information: ' + str(e)})
    ...
    for operator in users:
        ...
        try:
            ...
            if 'view_mode' not in operator:
                ...
                operator['view_mode'] = "operator"
            map = await db_objects.create(OperatorOperation, operator=op, operation=operation, view_mod=operator['view_mode'])
            await db_objects.create(db_model.OperationEventLog, operation=operation,
                                    message="{} added {} to operation as {}".format(adder.username, op.username, operator['view_mode']))
        except Exception as e:
    ...
    if operation.name in user['admin_operations'] or user['admin']:
        ...
        if 'admin' in data and data['admin'] != operation.admin.username:
            try:
                ...
                await db_objects.create(db_model.OperationEventLog, operation=operation,
                                        message="{} made {} the operation admin".format(modifier.username, new_admin.username))
            except Exception as e:
                print(e)
        ...
        if 'add_members' in data:
            for new_member in data['add_members']:
                try:
                    query = await db_model.operator_query()
                    operator = await db_objects.get(query, username=new_member['username'])
                    ...
                    elif new_member['view_mode'] not in ["operator", "developer", "spectator"]:
                        new_member['view_mode'] = "operator"
                    try:
                        map = await db_objects.get(OperatorOperation, operator=operator, operation=operation)
                        map.view_mode = new_member['view_mode']
                        await db_objects.update(map)
                        await db_objects.create(db_model.OperationEventLog, operation=operation,
                                                message="{} updated {} view mode to {}".format(modifier.username, operator.username, new_member['view_mode']))
                    except Exception as e:
                        map = await db_objects.create(OperatorOperation, operator=operator, operation=operation,
                                                      view_mode=new_member['view_mode'])
                        await db_objects.create(db_model.OperationEventLog, operation=operation,
                                                message="{} added {} to operation with view mode {}".format(modifier.username, operator.username, new_member['view_mode']))
                except Exception as e:
            ...
            for old_member in data['remove_members']:
                try:
                    ...
                    if operation.admin.username != operator.username:
                        ...
                        await db_objects.create(db_model.OperationEventLog, operation=operation,
                                                message="{} removed {} from operation".format(modifier.username, operator.username))
                except Exception as e:
        ...
        if 'webhook' in data:
            if (data['webhook'] == "" or data['webhook'] is None) and (operation.webhook is not None and operation.webhook != ""):
                ...
                if operation.webhook is not None:
                    await db_objects.create(db_model.OperationEventLog, operation=operation,
                                            message="{} added operation webhook: {}".format(modifier.username, data['webhook']))
        if 'complete' in data:
...
async def create_operation(request, user):
    ...
    if user['admin']:
        ...
        try:
            ...
            query = await db_model.operator_query()
            new_admin = await db_objects.get(query, username=data['admin'])
            operation = await db_objects.create(db_model.Operation, name=data['name'], admin=new_admin,
                                                AESPSK=await create_key_AES256())
            await db_objects.create(db_model.OperationEventLog, operation=operation,
                                    message="{} created operation {}".format(modifier.username, data['name']))
            await db_objects.create(db_model.OperationEventLog, operation=operation,
                                    message="{} made {} the operation admin".format(modifier.username, new_admin.username))
        except Exception as e:
            print(e)
            return json({'status': 'error', 'error': "Error creating operation: " + str(e)})
        added_members = []
        if 'members' in data:
            ...
            for m in data['members']:
                if m['username'] == data['admin']:
                    found_admin = True
            if not found_admin:
                data['members'].append({"username": data['admin'], "view_mode": "operator"})
        ...
        if 'webhook' in data:
            if (data['webhook'] == "" or data['webhook'] is None) and (operation.webhook is not None and operation.webhook != ""):
                ...
                await db_objects.create(db_model.OperationEventLog, operation=operation,
                                        message="{} added operation webhook: {}".format(modifier.username, data['webhook']))
        await db_objects.update(operation)
        return json({'status': 'success', 'members': added_members, **operation.to_json()})
    else:
        return json({'status': 'error', 'error': 'not authorized to make the change'})
...
@mythic.route(mythic.config['API_BASE'] + "/operations/disabled_commands_profiles", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
@mythic.route(mythic.config['API_BASE'] + "/operations/disabled_commands_profile", methods=['POST'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def create_disabled_commands_profile(request, user):
    ...
    try:
        if not user['admin']:
            return json({"status": 'error', 'error': 'Must be a Mythic admin to create disabled command profiles'})
        data = request.json
        added_acl = []
...
@mythic.route(mythic.config['API_BASE'] + "/operations/disabled_commands_profiles/<profile:string>", methods=['DELETE'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
async def delete_disabled_commands_profile(request, user, profile):
    ...
    try:
        ...
        if not user['admin']:
            return json({'status': 'error', 'error': 'Must be a Mythic admin to delete command profiles'})
        query = await db_model.disabledcommandsprofile_query()
...
@mythic.route(mythic.config['API_BASE'] + "/operations/disabled_commands_profile", methods=['PUT'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
