...
async def get_one_operator(request, oid, user):
    ...
    try:
        ...
        if op.username == user['username'] or user['view_mode'] != 'spectator':
            ...
            return json({'status': 'error', 'error': 'Spectators cannot query for specific users'})
    except:
        print("Failed to get operator")
        return json({'status': 'error', 'error': 'failed to get operator'})


@mythic.route(mythic.config['API_BASE'] + "/operators/config/<name:string>", methods=['GET'])
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
...
async def remove_operator(request, oid, user):
    ...
    try:
        ...
        if oid == 1:
            return json({'status': 'error', 'error': 'cannot delete the default account so you always have a way in'})
    except Exception as e:
        print(e)
