...
import json as js
import asyncio
from app.database_models.model import Callback, Payload, PayloadType, C2Profile, Credential, FileMeta, Task, TaskArtifact
        ...
        async with aiopg.create_pool(mythic.config['DB_POOL_CONNECT_STRING']) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    tasks_with_all_info = await db_objects.execute(query)
        ...
        async with aiopg.create_pool(mythic.config['DB_POOL_CONNECT_STRING']) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    responses_with_tasks = await db_objects.execute(query)
        ...
        async with aiopg.create_pool(mythic.config['DB_POOL_CONNECT_STRING']) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    wrappedpayloadtypesquery = await db_model.wrappedpayloadtypes_query()
                    ...
                    for p in payloadtypes:
                        wrappedpayloadtypes = await db_objects.execute(
                            wrappedpayloadtypesquery.where(db_model.WrappedPayloadTypes.wrapper == p))
                        await ws.send(js.dumps({**p.to_json(), "wrapped_payload_types": [w.to_json() for w in wrappedpayloadtypes]}))
                    await ws.send("")
                    # now pull off any new payloads we got queued up while processing old data
                    query = await db_model.payloadtype_query()
                    while True:
        ...
        async with aiopg.create_pool(mythic.config['DB_POOL_CONNECT_STRING']) as pool:
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    # BEFORE WE START GETTING NEW THINGS, UPDATE WITH ALL OF THE OLD DATA
                    query = await db_model.command_query()
                    commands = await db_objects.execute(query.where(db_model.Command.deleted == False))
                    for c in commands:
                    ...
                    if user['current_operation'] != "":
                        ...
                        while True:
                            try:
                                ...
                                id = (msg.payload)
                                query = await db_model.filemeta_query()
                                f = await db_objects.get(query, id=id, operation=operation, is_screenshot=True)
                                query = await db_model.task_query()
                                task = await db_objects.get(query, id=f.task)
                                await ws.send(js.dumps({**task.callback.to_json(), **f.to_json(), 'callback_id': task.callback.id, 'comment': task.comment}))
                            except asyncio.QueueEmpty as e:
                    ...
                    if user['current_operation'] != "":
                        ...
                        while True:
                            try:
                                ...
                                id = (msg.payload)
                                query = await db_model.filemeta_query()
                                f = await db_objects.get(query, id=id, is_screenshot=True, operation=operation)
                                query = await db_model.task_query()
                                task = await db_objects.get(query, id=f.task)
                                await ws.send(js.dumps({**task.callback.to_json(), **f.to_json(), 'callback_id': task.callback.id, 'comment': task.comment}))
                            except asyncio.QueueEmpty as e:
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        query = await db_model.filemeta_query()
                        files = await db_objects.execute(query.where(
                            (FileMeta.operation == operation) & (FileMeta.is_screenshot == False) & (FileMeta.is_payload == False)
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        query = await db_model.filemeta_query()
                        files = await db_objects.execute(query.where(
                            (FileMeta.operation == operation) & (FileMeta.deleted == False) & (FileMeta.is_payload == False) & (FileMeta.task == None)
                        ).order_by(FileMeta.id))
                        for f in files:
                            if f.task is None:
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        creds = await db_objects.execute(query.where( (Credential.operation == operation) & (Credential.deleted == False)))
                        for c in creds:
                            await ws.send(js.dumps({**c.to_json()}))
                        await ws.send("")
                        # now pull off any new payloads we got queued up while processing old data
                        while True:
                            try:
                                ...
                                try:
                                    query = await db_model.credential_query()
                                    c = await db_objects.get(query, id=id, operation=operation, deleted=False)
                                    await ws.send(js.dumps({**c.to_json()}))
                                except Exception as e:
                                    pass  # we got a file that's just not part of our current operation, so move on
                            except asyncio.QueueEmpty as e:
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        creds = await db_objects.execute(credquery.where((Credential.operation == operation) & (Credential.deleted == False)))
                        for c in creds:
                        ...
                        for p in payloads:
                            if p.wrapped_payload is not None:
                                ...
                                cur_payload = p
                            c2profiles = await db_objects.execute(c2profilepayloadquery.where(db_model.PayloadC2Profiles.payload == cur_payload))
                            supported_profiles = []
                            for c2p in c2profiles:
                                profile_info = {"name": c2p.c2_profile.name, "is_p2p": c2p.c2_profile.is_p2p, "parameters": {}}
                                c2profiledata = await db_objects.execute(c2profileparameterinstancequery.where(
                                    (db_model.C2ProfileParametersInstance.payload == cur_payload) &
                                    (db_model.C2ProfileParametersInstance.c2_profile == c2p.c2_profile)
                                ))
                                for c in c2profiledata:
                                    profile_info['parameters'][c.c2_profile_parameters.name] = c.value
                        ...
                        for p in payloadonhost:
                            ...
                            c2profiles = await db_objects.execute(
                                    c2profilepayloadquery.where(db_model.PayloadC2Profiles.payload == cur_payload))
                            supported_profiles = []
                            for c2p in c2profiles:
                            ...
                            try:
                                ...
                                try:
                                    if 'credential' in msg.channel:
                                        ...
                                        if payloadonhost.payload.wrapped_payload is not None:
                                            cur_payload = payloadonhost.payload.wrapped_payload
                                        else:
                                        ...
                                        payload = await db_objects.get(payloadquery.where(
                                            (Payload.operation == operation) & (Payload.id == id) & (Payload.deleted == False) & (Payload.build_phase == "success")
                                        ))
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        initial_events = await db_objects.execute(query.where(
...
@mythic.websocket('/ws/events_new/current_operation')
@inject_user()
@scoped(['auth:user', 'auth:apitoken_user'], False)  # user or user-level api token are ok
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        ...
                        profiles = await db_objects.execute(c2query.where(
                        ...
                        while True:
                            try:
                                ...
                                i = await db_objects.get(query, id=id, operation=operation)
                                if i.source.id == i.destination.id:
                                    await ws.send(js.dumps({**i.to_json(),
                                        "destination": js.dumps({
                                            "id": "c" + str(i.c2_profile.id)
            ...
            async with pool.acquire() as conn:
                async with conn.cursor() as cur:
                    ...
                    if user['current_operation'] != "":
                        query = await db_model.operation_query()
                        operation = await db_objects.get(query, name=user['current_operation'])
                        from app.api.file_browser_api import get_filebrowser_tree_for_operation
                        burst = await get_filebrowser_tree_for_operation(user['current_operation'])
                        ...
                        while True:
                            try:
                                ...
                                if 'filemeta' in msg.channel:
                                    i = await db_objects.get(filequery, id=id, operation=operation)
                                    if i.file_browser is not None:
                                        i = await db_objects.get(query, id=i.file_browser, operation=operation)
                                        ij = i.to_json()
                                        ij['files'] = []
                                        for f in i.files:
                                            fjson = f.to_json()
