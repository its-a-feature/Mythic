...
var payloadtypes_table = new Vue({
    ...
    methods: {
        ...
        edit_commands_button: function(p){
            ...
            //console.log(p);
            if(commandEdit_table.commands.length === 0){return}
        ...
        sync_payload_type_button: function(p){
            httpGetAsync("{{http}}://{{links.server_ip}}:{{links.server_port}}{{links.api_base}}/payloadtypes/" + p.id + "/files/sync", (response)=>{
                try{
                    let data = JSON.parse(response);
                    if(data['status'] === 'success'){
                        alertTop("success", "Sent sync message to container...");
                    }else{
            ...
            for(let i = 0; i < payloadtypes_table.payloadtypes.length; i++){
                for(let j = 0; j < payloadtypes_table.payloadtypes[i]['commands'].length; j++){
                    if(data['id'] ===  payloadtypes_table.payloadtypes[i]['commands'][j]['id']){
                        ...
                        commandEdit_table.selected_command = data;
                        return;
                    }
                }
            }
