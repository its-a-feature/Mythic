...
var callback_table = new Vue({
    ...
    methods: {
        ...
        interact_button: function(callback){
            ...
            if(!(callback.id in task_data.meta)){return;}
        ...
        remove_callback: function(callback){
            if(meta[callback.id] === undefined){stop_getting_callback_updates(callback.id, true);return}
                ...
                try{
                    ...
                    if(data['status'] === 'success'){
                        for(let i = 0; i < data['proxies'].length; i++){
                            if(!callback_table.callbacks.hasOwnProperty(data['proxies'][i]['id'])){
        ...
        get_egress_callback: function(callback){
          // given a callback, see if it's linked, and if so, what the egress node is
        ...
        apply_filter: function(callback){
            if(this.filter.includes(":")){
                ...
                if(callback.hasOwnProperty(pieces[0])){
                    if(callback[pieces[0]] === null){return false}
                ...
                for(let i in selected_list){
                    for(let j = 0; j < task_data.ptype_cmd_params[callbacks[selected_list[i]]['payload_type']].length; j++){
                       if(task_data.ptype_cmd_params[callbacks[selected_list[i]]['payload_type']][j]['is_exit']){
                            httpGetAsync("{{http}}://{{links.server_ip}}:{{links.server_port}}{{links.api_base}}/tasks/callback/" + selected_list[i], null, "POST", {"command":task_data.ptype_cmd_params[callbacks[selected_list[i]]['payload_type']][j].cmd,"params":""});
                       }
                    }
                }
                group_modify.callbacks = [];
        ...
        graph_node_update: function(){
            ...
            this.graph_view_pieces.json_data.nodes.forEach(function(d, i){
                if(document.getElementById("avatar" + d.payload_type) == null) {
                  let tmp = defs.append("svg:pattern")
            ...
            function update_collapse(source){
                ...
                let link = callback_table.tree_view_pieces.gDraw.select("#links").selectAll(".link")
                    ...
                    .style('stroke-width', function(d){
                        if(d.children){
                            return d3.max(d.children, function(x) { return 1 + (0.1 * x.depth);});
                        }else if(d._children){
                            return d3.max(d._children, function(x) { return 1 + (0.1 * x.depth);});
                        }else{
                            return 1;
                        }
      ...
      if(callback_table.graph_view_pieces.json_data['links'][i]['id'] === cb['id']){
        ...
        if(cb['end_timestamp'] === null){
          // update something else about the link
            ...
            let found_t = false;
            for(let i = 0; i < callback_table.graph_view_pieces.json_data.nodes.length; i++){
                if(cb['source'] === callback_table.graph_view_pieces.json_data.nodes[i]['id']){
                    found_s = true;
                    }
                if(cb['target'] === callback_table.graph_view_pieces.json_data.nodes[i]['id']){
                    found_t = true;
                }
            }
            if(!found_s){
...
var task_data = new Vue({
    ...
    methods:{
                    ...
                    //if we find our command that was typed
                        ...
                        if(params.length === 0 && this.ptype_cmd_params[callbacks[data['cid']]['payload_type']][i]['params'].length !== 0){
                            ...
                            for(let j = 0; j < this.ptype_cmd_params[callbacks[data['cid']]['payload_type']][i]['params'].length; j++){
                                let blank_vals = {"string_value": "", "credential_value":"", "credential_id": 0, "number_value": -1, "choice_value": "", "choicemultiple_value": [], "boolean_value": false, "array_value": [], 'payloadlist_value': "", 'agentconnect_c2profile': -1, 'agentconnect_host': "", "agentconnect_payload": "", "payloads": []};
                                ...
                                if(param.type === 'PayloadList'){
                                    ...
                                    if(supported_agents.indexOf("") !== -1){supported_agents.splice(supported_agents.indexOf(""), 1);}
                                    if( supported_agents.length === 0){param.payloads = params_table.payloads;}
                                    else{
                                        for(let i = 0; i < params_table.payloads.length; i++){
                                            if(supported_agents.includes(params_table.payloads[i]['payload_type'])){
                                                param.payloads.push(params_table.payloads[i]);
                                            }
                                        }
                                    }
                                }
                                    ...
                                    if(params_table.command_params[k]['type'] === "String"){  param_data[params_table.command_params[k]['name']] = params_table.command_params[k]['string_value']; }
                                        ...
                                        params_table.credentials.forEach(function(x){
                                            if(x['id'] === params_table.command_params[k]['credential_id']){
                                                param_data[params_table.command_params[k]['name']] = {'account': x['account'],
                                                'realm': x['realm'],
        ...
        callback_total_badges: function(metadata){
            if(all_tasks[metadata.id] === undefined){return 0}
            ...
            this.ptype_cmd_params[callbacks[task_data.callback]['payload_type']].forEach(function(x){
                if(x['is_download_file'] === true){
                    ...
                    httpGetAsync("{{http}}://{{links.server_ip}}:{{links.server_port}}{{links.api_base}}/tasks/callback/" + task_data.callback,post_task_callback_func, "POST",
                        {"command":x['cmd'],"params":JSON.stringify({"host": data['host'], "path": data['parent_path'], "file": data['name']})});
        ...
        task_list_file_refresh: function(){
            ...
            if(!meta.hasOwnProperty(this.callback)){
    ...
    computed: {
        ...
        get_proxies: function(){
            return Object.keys(callback_table.callbacks).filter(function(elem, index, array) {
...
var params_table = new Vue({
    ...
    methods:{
        ...
        submit_manually_add_payloadonhost: function(param){
            ...
            }, "POST", {"host": param.manually_add_payloadonhost_hostname, "uuid": param.manually_add_payloadonhost_template});
            Vue.set(param, "manually_add_payloadonhost", false);
            Vue.set(param, "manually_add_payloadonhost_hostname", "");
            Vue.set(param, "manually_add_payloadonhost_template", "");
        },
        delete_host: function(param){
             if(param.agentconnect_host === ""){return}
...
function register_new_command_info(response){
    ...
    if(data['status'] === "success"){
        ...
        if(data['commands'].length > 0){
            data['commands'].push({"cmd": "help", "params":[], "help_cmd": "help [command]", "description": "get the description and cmd  usage of a command"});
            data['commands'].push({"cmd": "set", "params":[], "help_cmd": "set keyword value. Keywords are 'parent' and 'description'", "description": "set certain properties of a callback like the parent callback or the description"});
            data['commands'].push({"cmd": "tasks", "params":[], "help_cmd": "tasks", "description": "query the mythic server for tasks that have been issued but not picked up by the agent in this callback"});
            data['commands'].push({"cmd": "clear", "params":[], "help_cmd": "clear [all|task_num]", "description": "clear a task from the server before an agent has picked it up"});
...
function add_new_response(rsp, from_websocket){
    try{
        ...
        if(rsp['task']['callback'] in all_tasks){
            ...
            if(browser_scripts.hasOwnProperty(rsp['task']['command_id']) && all_tasks[rsp['task']['callback']][rsp['task']['id']]['use_scripted']){
...
function startwebsocket_commands(){
    ...
    ws.onmessage = function(event){
        if(event.data === "no_operation"){
            ...
            if(task_data.ptype_cmd_params[data['payload_type']] === undefined){return}
...
function startwebsocket_events() {
    ...
    ws.onmessage = function (event) {
        if(event.data === "no_operation"){
            ...
            alertTop(data['level'], message, 10, data['operator']);
        }
    };
    ws.onclose = function (event) {
        wsonclose(event);
    ...
    ws.onmessage = function (event) {
        if(event.data === "no_operation"){
            ...
            if(!initial_group_done){
                ...
                if(data['host'] === undefined || data['host'].length === 0){return}
                if(!meta['file_browser'].hasOwnProperty(data['host'])){
                    meta['file_browser'][data['host']] = {"data": {
                        "name": data['host'],
                        "host": data['host']
                        }, "children": []};
                }
                ...
                if(data['parent'] === null){
                    for(let i = 0; i < meta['file_browser'][data['host']]['children'].length; i++){
                        if(meta['file_browser'][data['host']]['children'][i].hasOwnProperty(data['name'])){
