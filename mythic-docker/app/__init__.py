...
# --------------------------------------------
# --------------------------------------------
mythic_admin_user = settings.get("mythic_admin_user", "mythic_admin")
mythic_admin_password = settings.get("mythic_admin_password", "mythic_password")
default_operation_name = settings.get("default_operation_name", "Operation Chimera")
nginx_port = str(settings.get("nginx_port", "7443"))
listen_port = str(settings.get("SERVER_PORT", "17443"))
allowed_ip_blocks = settings.get("allowed_ip_blocks", ["0.0.0.0/0"])
...
if db_pass is None:
    logging.exception("No POSTGRES_PASSWORD specified in config and no MYTHIC_POSTGRES_PASSWORD in environment variables")
    sys.exit(1)
...
rabbitmq_password = settings.get("RABBITMQ_PASSWORD", "mythic_password")
...
max_log_count = 1
valid_payload_container_version_bounds = [6, 6]
