from app import mythic, dbloop, mythic_db, db_objects, use_ssl, listen_port, listen_ip, ssl_cert_path, ssl_key_path, keep_logs
import asyncio
import ssl
...
if __name__ == "__main__":
    try:
        ...
        if use_ssl:
            ...
            server = mythic.create_server(host=listen_ip, port=listen_port, ssl=context, debug=False, return_asyncio_server=True, access_log=keep_logs)
        else:
            server = mythic.create_server(host=listen_ip, port=listen_port, debug=False, return_asyncio_server=True, access_log=keep_logs)
