...
type GetC2ConfigCheckInput struct {
	Input GetC2ConfigCheck `json:"input" binding:"required"`
}

type GetC2ConfigCheck struct {
	PayloadUUID string `json:"uuid" binding:"required"`
}

type GetC2ConfigCheckResponse struct {
	Status string `json:"status"`
	Output string `json:"output"`
	Error  string `json:"error"`
}

func C2ProfileConfigCheckWebhook(c *gin.Context) {
	// get variables from the POST request
	var input GetC2ConfigCheckInput
	output := ""
	...
	for c2ProfileName, c2ProfileGroup := range parametersMap {
		...
		if c2ConfigCheckMessageResponse, err := rabbitmq.RabbitMQConnection.SendC2RPCConfigCheck(rabbitmq.C2ConfigCheckMessage{
			...
			output += fmt.Sprintf("Failed Configuration Check for %s\n%s\n", c2ProfileName, c2ConfigCheckMessageResponse.Error)
		} else {
