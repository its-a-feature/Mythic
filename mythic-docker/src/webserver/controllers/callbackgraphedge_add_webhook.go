...
func CallbackgraphedgeAddWebhook(c *gin.Context) {
	...
	err = rabbitmq.AddEdgeByDisplayIds(input.Input.SourceCallbackId, input.Input.DestinationCallbackId, input.Input.C2ProfileName, operatorOperation, authentication.RabbitMQAuthContextFromGin(c))
	if err != nil {
