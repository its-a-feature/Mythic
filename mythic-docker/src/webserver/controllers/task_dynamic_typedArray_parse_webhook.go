...
func PayloadTypeDynamicTypedArrayParseWebhook(c *gin.Context) {
	...
	if err != nil {
		logging.LogError(err, "Failed to resolve callback display id for typed array parse")
		c.JSON(http.StatusOK, PayloadTypeDynamicTypedArrayParseResponse{
			Status: rabbitmq.PT_TYPEDARRAY_PARSE_STATUS_ERROR,
	...
	payloadtypeDynamicQueryResponse, err := rabbitmq.RabbitMQConnection.SendPtRPCTypedArrayParse(rabbitmq.PTRPCTypedArrayParseMessage{
