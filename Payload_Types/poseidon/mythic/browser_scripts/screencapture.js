function(task, responses){
  ...
  for(let i = 0; i < responses.length; i+=2){
  	if( i+1 < responses.length){
        ...
        output += "<div class='card'><div class='card-header border border-dark shadow'><a class='btn stretched-link' type='button' data-toggle='collapse' data-target='#task" + task['id'] + i + "screencapture' aria-expanded='false' aria-controls='task" + task['id'] + "screencapture'>Finished <font color='red'>Screencapture " + task['params'] + "</font>. Click to view</div>";
