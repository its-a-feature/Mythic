...
class ServiceWrapper(PayloadType):
    ...
    build_parameters = {
        ...
        "arch": BuildParameter(name="arch", parameter_type=BuildParameterType.ChooseOne, choices=["x64", "Any CPU"], default_value="x64", description="Target architecture"),
        "config": BuildParameter(name="config", parameter_type=BuildParameterType.ChooseOne, choices=["Release"], default_value="Release", description="Configuration")
    }
    c2_profiles = []
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            working_path = PurePath(agent_build_path) / "WindowsService1" / "Resources" / "loader.bin"
            ...
            if stderr:
                output += f'[stderr]\n{stderr.decode()}'
            output_path = PurePath(agent_build_path) / "WindowsService1" / "bin" / self.get_parameter('config') / "WindowsService1.exe"
